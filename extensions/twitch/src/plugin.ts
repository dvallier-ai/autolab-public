/**
 * Twitch channel plugin for AutoLab.
 *
...
import { buildChannelConfigSchema } from "autolab/plugin-sdk";
import type {
 ...
 * for AutoLab. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AutoLabConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AutoLabConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AutoLabConfig): boolean => {
