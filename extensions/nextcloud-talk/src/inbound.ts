...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AutoLabConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AutoLabConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AutoLabConfig);
