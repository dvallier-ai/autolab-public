...
metadata: { "autolab": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AutoLab `sessions_spawn`
- **File I/O** == AutoLab `read`/`write`
- **Remote fetch** == AutoLab `web_fetch` (or `exec` with curl when POST is required)
