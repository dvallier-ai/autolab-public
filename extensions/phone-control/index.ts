import type { AutoLabPluginApi, AutoLabPluginService } from "autolab/plugin-sdk";
...
function normalizeDenyList(cfg: AutoLabPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AutoLabPluginApi["config"]): string[] {
...
export default function register(api: AutoLabPluginApi) {
  ...
  const timerService: AutoLabPluginService = {
