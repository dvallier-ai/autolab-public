...
import { approveDevicePairing, listDevicePairing } from "autolab/plugin-sdk";
...
function resolveGatewayPort(cfg: AutoLabPluginApi["config"]): number {
  const envRaw =
    process.env.AUTOLAB_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: AutoLabPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: AutoLabPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: AutoLabPluginApi): Promise<ResolveUrlResult> {
