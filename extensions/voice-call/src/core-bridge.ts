...
function resolveAutoLabRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.AUTOLAB_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AUTOLAB_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAutoLabRoot(), "dist", "extensionAPI.js");
