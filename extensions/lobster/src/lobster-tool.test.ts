...
import type { AutoLabPluginApi, AutoLabPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "autolab-lobster-plugin-") {
...
function fakeApi(overrides: Partial<AutoLabPluginApi> = {}): AutoLabPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AutoLabPluginToolContext> = {}): AutoLabPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AutoLabPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
