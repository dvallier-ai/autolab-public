...
AutoLab extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
autolab channels login --channel zalouser
...
```bash
autolab message send --channel zalouser --target <threadId> --message "Hello from AutoLab!"
```
...
```bash
autolab channels login --channel zalouser              # Login via QR
autolab channels login --channel zalouser --account work
autolab channels status --probe
autolab channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
autolab directory self --channel zalouser
autolab directory peers list --channel zalouser --query "name"
autolab directory groups list --channel zalouser --query "work"
autolab directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
autolab message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
autolab message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `autolab channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
autolab channels login --channel zalouser --account work
autolab message send --channel zalouser --account work --target <id> --message "Hello"
