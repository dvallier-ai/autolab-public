...
`autolab.invoke --each`) without adding new AutoLab code per workflow.
...
This extension depends on AutoLab internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AutoLab extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.autolab/extensions` as a standalone plugin directory.
