...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/autolab/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/autolab");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/autolab/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AUTOLAB_CONTROL_UI_BASE_PATH__ = "/autolab";
    const app = mountApp("/autolab/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/autolab");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/autolab/sessions");
  });
