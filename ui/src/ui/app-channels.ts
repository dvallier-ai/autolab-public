...
export async function handleWhatsAppStart(host: AutoLabApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AutoLabApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AutoLabApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AutoLabApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AutoLabApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AutoLabApp): string {
...
function resolveGatewayHttpAuthHeader(host: AutoLabApp): string | null {
...
function buildGatewayHttpHeaders(host: AutoLabApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AutoLabApp) {
...
export function handleNostrProfileToggleAdvanced(host: AutoLabApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AutoLabApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AutoLabApp) {
  const state = host.nostrProfileFormState;
