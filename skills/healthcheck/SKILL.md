---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AutoLab deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AutoLab cron scheduling for periodic checks, or version status checks on a machine running AutoLab (laptop, workstation, Pi, VPS).
---

# AutoLab Host Hardening

## Overview

Assess and harden the host running AutoLab, then align it to a user-defined risk tolerance without breaking access. Use AutoLab security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AutoLab changes the host firewall, SSH, or OS updates; it does not.
...
5. AutoLab gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AutoLab can access sensitive data.
...
### 2) Run AutoLab security audits (read-only)

As part of the default read-only checks, run `autolab security audit --deep`. Only offer alternatives if the user requests them:

1. `autolab security audit` (faster, non-probing)
2. `autolab security audit --json` (structured output)

Offer to apply AutoLab safe defaults (numbered):

1. `autolab security audit --fix`

Be explicit that `--fix` only tightens AutoLab defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AutoLab version/update status (read-only)

As part of the default read-only checks, run `autolab update status`.
...
- Credential hygiene notes (location of AutoLab creds, prefer disk encryption)
...
After AutoLab install or first hardening pass, run at least one baseline audit and version check:

- `autolab security audit`
- `autolab security audit --deep`
- `autolab update status`

Ongoing monitoring is recommended. Use the AutoLab cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `autolab cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `autolab update status`
...
Before creating, `autolab cron list` and match on exact `name`. If found, `autolab cron edit <id> ...`.
If not found, `autolab cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `autolab update status` (preferred for source checkouts and channels)
2. `npm view autolab version` (published npm version)

## AutoLab command accuracy

Use only supported commands and flags:

- `autolab security audit [--deep] [--fix] [--json]`
- `autolab status` / `autolab status --deep`
- `autolab health --json`
- `autolab update status`
- `autolab cron add|list|runs|run`

Do not invent CLI flags or imply AutoLab enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AutoLab compaction:
