...
import type { AutoLabConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("autolab-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("autolab-bin-sherpa-");
      const modelDir = await makeTempDir("autolab-sherpa-model-");
      ...
      const cfg: AutoLabConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("autolab-bin-whispercpp-");
      const modelDir = await makeTempDir("autolab-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("autolab-bin-gemini-");
      ...
      const cfg: AutoLabConfig = { tools: { media: { image: {} } } };
