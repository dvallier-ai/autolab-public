...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `autolab-e2e-${name}-`));
  const configDir = path.join(homeDir, ".autolab");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          AUTOLAB_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        AUTOLAB_GATEWAY_TOKEN: inst.gatewayToken,
        AUTOLAB_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AUTOLAB_GATEWAY_PORT: String(gwA.port),
          AUTOLAB_GATEWAY_TOKEN: gwA.gatewayToken,
          AUTOLAB_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AUTOLAB_GATEWAY_PORT: String(gwB.port),
          AUTOLAB_GATEWAY_TOKEN: gwB.gatewayToken,
          AUTOLAB_GATEWAY_PASSWORD: "",
        }),
