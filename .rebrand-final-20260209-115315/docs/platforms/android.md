...
- You can run the CLI (`autolab`) on the gateway machine (or via SSH).
...
1. Set up a DNS-SD zone (example `autolab.internal.`) on the gateway host and publish `_autolab-gw._tcp` records.
