...
```bash
autolab tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `autolab tui --deliver`
...
- Check the Gateway logs: `autolab logs --follow`.
- Confirm the agent can run: `autolab status` and `autolab models status`.
...
- No agents in picker: check `autolab agents list` and your routing config.
