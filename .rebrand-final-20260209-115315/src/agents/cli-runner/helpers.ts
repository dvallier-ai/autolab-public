...
/**
 * Cleanup suspended AutoLab CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: AutoLabConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-cli-images-"));
  const paths: string[] = [];
