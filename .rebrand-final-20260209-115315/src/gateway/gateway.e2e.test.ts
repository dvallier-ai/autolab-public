...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.AUTOLAB_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "autolab");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.autolab-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".autolab");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "autolab.json");
      ...
      try {
        ...
        process.env.AUTOLAB_CONFIG_PATH = prev.configPath;
        process.env.AUTOLAB_GATEWAY_TOKEN = prev.token;
        process.env.AUTOLAB_SKIP_CHANNELS = prev.skipChannels;
        process.env.AUTOLAB_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.AUTOLAB_SKIP_CRON = prev.skipCron;
        process.env.AUTOLAB_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.AUTOLAB_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.AUTOLAB_STATE_DIR = prev.stateDir;
      process.env.AUTOLAB_CONFIG_PATH = prev.configPath;
      process.env.AUTOLAB_GATEWAY_TOKEN = prev.token;
      process.env.AUTOLAB_SKIP_CHANNELS = prev.skipChannels;
      process.env.AUTOLAB_SKIP_GMAIL_WATCHER = prev.skipGmail;
