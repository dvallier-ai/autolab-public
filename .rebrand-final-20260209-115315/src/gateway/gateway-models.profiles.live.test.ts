...
import type { AutoLabConfig, ModelProviderConfig } from "../config/types.js";
import { resolveAutoLabAgentDir } from "../agents/agent-paths.js";
...
import { ensureAutoLabModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.AUTOLAB_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.AUTOLAB_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.AUTOLAB_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AutoLabConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.AUTOLAB_AGENT_DIR,
  ...
  const hostAgentDir = resolveAutoLabAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-live-state-"));
  ...
  process.env.AUTOLAB_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.autolab-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAutoLabAgentDir();
  const sanitizedCfg: AutoLabConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "AutoLab live tool probe (local, safe): " +
    ...
    process.env.AUTOLAB_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.AUTOLAB_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAutoLabModelsJson(cfg);
      ...
      const rawModels = process.env.AUTOLAB_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.autolab-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
