...
export function isCommandEnabled(cfg: AutoLabConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AutoLabConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AutoLabConfig),
...
export function getCommandDetection(_cfg?: AutoLabConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AutoLabConfig) {
