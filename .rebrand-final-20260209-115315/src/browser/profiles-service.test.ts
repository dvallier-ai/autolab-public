...
vi.mock("./chrome.js", () => ({
  resolveAutoLabUserDataDir: vi.fn(() => "/tmp/autolab-test/autolab/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "autolab-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveAutoLabUserDataDir).mockReturnValue(userDataDir);
