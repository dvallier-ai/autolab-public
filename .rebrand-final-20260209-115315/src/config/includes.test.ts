...
const ETC_AUTOLAB_DIR = path.join(ROOT_DIR, "etc", "autolab");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "autolab.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAutoLabPath(...parts: string[]) {
  return path.join(ETC_AUTOLAB_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAutoLabPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "autolab.json"))).toEqual({
      shared: true,
    });
  });
});
