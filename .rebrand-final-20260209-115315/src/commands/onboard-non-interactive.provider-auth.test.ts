...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.AUTOLAB_GATEWAY_PASSWORD,
    disableConfigCache: process.env.AUTOLAB_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("AUTOLAB_STATE_DIR", prev.stateDir);
  restoreEnvVar("AUTOLAB_CONFIG_PATH", prev.configPath);
  restoreEnvVar("AUTOLAB_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("AUTOLAB_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("AUTOLAB_SKIP_CRON", prev.skipCron);
  restoreEnvVar("AUTOLAB_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("AUTOLAB_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("AUTOLAB_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("AUTOLAB_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("autolab-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("autolab-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
