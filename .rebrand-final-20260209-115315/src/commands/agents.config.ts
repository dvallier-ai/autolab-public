...
export function listAgentEntries(cfg: AutoLabConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AutoLabConfig, agentId: string) {
...
function resolveAgentModel(cfg: AutoLabConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AutoLabConfig): AgentSummary[] {
