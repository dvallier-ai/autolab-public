...
const SEARCH_TOOL = "https://docs.autolab.ai/mcp.SearchAutoLab";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.autolab.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('autolab docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.autolab.ai/");
      runtime.log(`Search: ${formatCliCommand('autolab docs "your query"')}`);
    }
    return;
  }
