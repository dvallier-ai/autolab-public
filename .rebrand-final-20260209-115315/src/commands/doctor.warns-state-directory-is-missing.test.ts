...
beforeEach(() => {
  ...
  resolveAutoLabPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AUTOLAB_STATE_DIR;
  originalUpdateInProgress = process.env.AUTOLAB_UPDATE_IN_PROGRESS;
  process.env.AUTOLAB_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.AUTOLAB_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AUTOLAB_UPDATE_IN_PROGRESS;
  } else {
    process.env.AUTOLAB_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveAutoLabPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadAutoLabPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/autolab-root.js", () => ({
  resolveAutoLabPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.AUTOLAB_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when AUTOLAB_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/autolab.json",
      exists: true,
      raw: "{}",
    ...
    process.env.AUTOLAB_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
