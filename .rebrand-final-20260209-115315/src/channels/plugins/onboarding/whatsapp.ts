...
function setWhatsAppDmPolicy(cfg: AutoLabConfig, dmPolicy: DmPolicy): AutoLabConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: AutoLabConfig, allowFrom?: string[]): AutoLabConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: AutoLabConfig, selfChatMode: boolean): AutoLabConfig {
...
async function detectWhatsAppLinked(cfg: AutoLabConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so AutoLab can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for AutoLab" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("autolab channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
