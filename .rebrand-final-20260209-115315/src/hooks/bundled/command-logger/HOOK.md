...
2. **Appends to log file** - Writes a JSON line to `~/.autolab/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.autolab/logs/commands.log ~/.autolab/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/autolab`:
   ```
   /home/username/.autolab/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
