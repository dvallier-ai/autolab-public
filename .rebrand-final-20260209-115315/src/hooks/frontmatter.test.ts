...
`;
    ...
    expect(parsed.autolab.emoji).toBe("💾");
    expect(parsed.autolab.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.autolab.emoji).toBe("📝");
    expect(parsed.autolab.events).toEqual(["command"]);
    expect(parsed.autolab.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.autolab.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"autolab": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"autolab": {"events": ["test"]}}');
  });
...
describe("resolveAutoLabMetadata", () => {
  it("extracts autolab metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAutoLabMetadata(frontmatter);
  ...
  it("returns undefined when autolab key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        autolab: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with AutoLab" },
            { id: "npm", kind: "npm", package: "@autolab/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@autolab/hook");
  });
    ...
    "autolab":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with AutoLab" }],
      },
  }
---
...
`;
    ...
    const autolab = resolveAutoLabMetadata(frontmatter);
    expect(autolab).toBeDefined();
    expect(autolab?.emoji).toBe("💾");
    expect(autolab?.events).toEqual(["command:new"]);
    expect(autolab?.requires?.config).toEqual(["workspace.dir"]);
    expect(autolab?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const autolab = resolveAutoLabMetadata(frontmatter);
    expect(autolab?.emoji).toBe("disk");
    expect(autolab?.events).toEqual(["command:new"]);
  });
});
