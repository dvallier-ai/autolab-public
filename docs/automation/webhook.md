...
read_when:
  ...
  - Wiring external systems into AutoLab
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your AutoLab config directory (typically `~/.autolab/hooks/transforms`).
...
- `autolab webhooks gmail setup` writes `hooks.gmail` config for `autolab webhooks gmail run`.
