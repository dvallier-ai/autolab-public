...
- Jobs persist under `~/.autolab/cron/` so restarts don’t lose schedules.
...
autolab cron list
autolab cron run <job-id>
autolab cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.autolab/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `autolab cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, AutoLab defaults to `announce`.
...
- Job store: `~/.autolab/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.autolab/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
autolab cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
autolab cron edit <jobId> --agent ops
autolab cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
autolab cron run <jobId>
autolab cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
autolab cron edit <jobId> \
...
```bash
autolab system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`autolab system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `AUTOLAB_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- AutoLab applies exponential retry backoff for recurring jobs after consecutive errors:
