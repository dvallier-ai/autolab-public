...
AutoLab features that can generate provider usage or paid API calls.
...
- `autolab status --usage` and `autolab channels list` show provider **usage windows**
...
AutoLab can pick up credentials from:
...
- `autolab status --usage`
- `autolab models status --json`
...
`autolab models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
