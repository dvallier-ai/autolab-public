...
This document explains how AutoLab manages sessions end-to-end:
...
AutoLab is designed around a single **Gateway process** that owns session state.
...
AutoLab persists sessions in two layers:
...
- Transcripts: `~/.autolab/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

AutoLab resolves these via `src/config/sessions.ts`.
...
AutoLab intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (AutoLab consumes the events, but Pi decides when to compact).
...
AutoLab also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, AutoLab bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, AutoLab leaves it alone.
...
- `autolab status` (CLI)
- `autolab sessions` / `sessions --json`
...
AutoLab supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- AutoLab strips/suppresses this in the delivery layer.

As of `2026.1.10`, AutoLab also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
AutoLab uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but AutoLab’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `autolab status`.
