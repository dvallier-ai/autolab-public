...
read_when:
  - You need detailed behavior for autolab onboard
...
This page is the full reference for `autolab onboard`.
...
<Steps>
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `autolab pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `autolab health`.
    - `autolab status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.autolab/.env` so launchd can read it.
...
- OAuth credentials: `~/.autolab/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.autolab/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`autolab onboard`](/cli/onboard)
