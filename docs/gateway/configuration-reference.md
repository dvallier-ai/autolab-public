...
description: "Complete field-by-field reference for ~/.danv-intel/autolab.json"
---

# Configuration Reference

Every field available in `~/.danv-intel/autolab.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — AutoLab uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `autolab doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["autolab-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `autolab plugins install @autolab/mattermost`.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@autolab"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `autolab.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, AutoLab auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, AutoLab assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.autolab/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.autolab/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@autolab"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:autolab`   | All built-in tools (excludes provider plugins)                                           |
...
AutoLab uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.autolab/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `AUTOLAB_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `autolab onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `autolab onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `autolab onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `autolab onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `autolab onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `autolab onboard --auth-choice minimax-api`.
...
- Loaded from `~/.autolab/extensions`, `<workspace>/.autolab/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or AUTOLAB_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `AUTOLAB_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.autolab-dev` + port `19001`), `--profile <name>` (uses `~/.autolab-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-autolab-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `AUTOLAB_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or AUTOLAB_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__autolab__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__autolab__/a2ui/`
...
- Also serves A2UI at `/__autolab__/a2ui/`.
...
- Hostname defaults to `autolab`. Override with `AUTOLAB_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.autolab/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `autolab dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.autolab/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${AUTOLAB_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.autolab/credentials/oauth.json`.
...
- Default log file: `/tmp/danv-intel/autolab-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `autolab doctor --fix` can strip unknown keys).
