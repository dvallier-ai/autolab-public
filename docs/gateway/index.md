...
- Replaces the legacy `gateway` command. CLI entry point: `autolab gateway`.
...
autolab gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.autolab/autolab.json` (or `AUTOLAB_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__autolab__/canvas/` from `~/.autolab/workspace/canvas`. Disable with `canvasHost.enabled=false` or `AUTOLAB_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `AUTOLAB_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `AUTOLAB_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.autolab.*` may still exist)
- Linux: `autolab-gateway-<profile>.service`
- Windows: `AutoLab Gateway (<profile>)`

Install metadata is embedded in the service config:

- `AUTOLAB_SERVICE_MARKER=autolab`
...
```bash
autolab --dev setup
autolab --dev gateway --allow-unconfigured
...
- `AUTOLAB_STATE_DIR=~/.autolab-dev`
- `AUTOLAB_CONFIG_PATH=~/.autolab-dev/autolab.json`
- `AUTOLAB_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.autolab/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `AUTOLAB_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `AUTOLAB_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `AUTOLAB_CONFIG_PATH`
- unique `AUTOLAB_STATE_DIR`
...
```bash
autolab --profile main gateway install
autolab --profile rescue gateway install
```

Example:

```bash
AUTOLAB_CONFIG_PATH=~/.autolab/a.json AUTOLAB_STATE_DIR=~/.autolab-a autolab gateway --port 19001
AUTOLAB_CONFIG_PATH=~/.autolab/b.json AUTOLAB_STATE_DIR=~/.autolab-b autolab gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `autolab health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `autolab`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `autolab gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.autolab.*` is cleaned up).
  - `autolab doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are AutoLab profile services.
  ...
  - Cleanup: `autolab gateway uninstall` (current service) and `autolab doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `autolab gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- AutoLab.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.autolab.*` labels still unload cleanly).
- To stop it cleanly, use `autolab gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `autolab gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `autolab gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

AutoLab installs a **systemd user service** by default on Linux/WSL2. We
...
`autolab gateway install` writes the user unit. `autolab doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/autolab-gateway[-<profile>].service`:

```
[Unit]
Description=AutoLab Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/autolab gateway --port 18789
Restart=always
RestartSec=5
Environment=AUTOLAB_GATEWAY_TOKEN=
...
```
systemctl --user enable --now autolab-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/autolab-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now autolab-gateway[-<profile>].service
```
...
- `autolab gateway health|status` — request health/status over the Gateway WS.
- `autolab message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `autolab agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `autolab gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `autolab gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `autolab gateway` and the legacy TCP control port.
