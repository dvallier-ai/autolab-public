...
`autolab doctor` is the repair + migration tool for AutoLab. It fixes stale
...
- Extra workspace dir detection (`~/autolab`).
...
- Sessions store + transcripts:
  - from `~/.autolab/sessions/` to `~/.autolab/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.autolab/agent/` to `~/.autolab/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.autolab/credentials/*.json` (except `oauth.json`)
  - to `~/.autolab/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `autolab doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.autolab` folders exist across
  home directories or when `AUTOLAB_STATE_DIR` points elsewhere (history can
...
offers to remove them and install the AutoLab service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named AutoLab gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `autolab doctor --generate-gateway-token` to force token
...
- `autolab doctor` prompts before rewriting supervisor config.
- `autolab doctor --yes` accepts the default repair prompts.
- `autolab doctor --repair` applies recommended fixes without prompts.
- `autolab doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `autolab gateway install --force`.
