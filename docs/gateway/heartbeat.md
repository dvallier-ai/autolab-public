...
- During heartbeat runs, AutoLab treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `autolab sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AutoLab skips the heartbeat run entirely (no model call).
...
```bash
autolab system event --text "Check for urgent follow-ups" --mode now
```
