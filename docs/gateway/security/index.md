...
## Quick check: `autolab security audit`
...
```bash
autolab security audit
autolab security audit --deep
autolab security audit --fix
```
...
- Tighten local perms (`~/.autolab` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

AutoLab is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, AutoLab also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.autolab/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.autolab/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.autolab/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.autolab/credentials/oauth.json`
...
`autolab security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${AUTOLAB_GATEWAY_PASSWORD}
```
...
AutoLab stores session transcripts on disk under `~/.autolab/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.autolab` (see the audit section below). If you need
...
AutoLab can refresh the skills list mid-session:
...
- If you install plugins from npm (`autolab plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.autolab/extensions/<pluginId>/` (or `$AUTOLAB_STATE_DIR/extensions/<pluginId>/`).
  - AutoLab uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
autolab pairing list <channel>
autolab pairing approve <channel> <code>
```
...
By default, AutoLab routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
AutoLab has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.autolab/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.autolab or your logs.”
...
4. **Re-run audit**
   - `autolab security audit --deep` and confirm the report is clean.
...
- `~/.danv-intel/autolab.json`: `600` (user read/write only)
- `~/.autolab`: `700` (user only)

`autolab doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `AUTOLAB_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__autolab__/canvas/` and `/__autolab__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_autolab-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `AUTOLAB_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `autolab doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `AUTOLAB_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `AUTOLAB_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), AutoLab
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. AutoLab verifies the identity by resolving the
...
- AutoLab will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.autolab/` (or `$AUTOLAB_STATE_DIR/`) may contain secrets or private data:

- `autolab.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `autolab status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@autolab", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.autolab/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `autolab` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only AutoLab clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default AutoLab limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `autolab gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `AUTOLAB_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/danv-intel/autolab-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.autolab/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + AutoLab version
