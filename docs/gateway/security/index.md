...
On Day 1, a friendly tester asked Labby to run `find ~` and share the output. Labby happily dumped the entire home directory structure to a group chat.
...
- `~/.autolab/autolab.json`: `600` (user read/write only)
...
1. Check Gateway logs: `/tmp/autolab/autolab-YYYY-MM-DD.log` (or `logging.file`).
...
flowchart TB
    A["Owner (Peter)"] -- Full trust --> B["AI (Labby)"]
