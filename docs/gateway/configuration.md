---
summary: "All configuration options for ~/.autolab/autolab.json with examples"
read_when:
...
AutoLab reads an optional **JSON5** config from `~/.autolab/autolab.json` (comments + trailing commas allowed).

If the file is missing, AutoLab uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.autolab/workspace`). You usually only need a config to:
...
AutoLab only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `autolab doctor`, `autolab logs`, `autolab health`, `autolab status`, `autolab service`, `autolab help`).
- Run `autolab doctor` to see the exact issues.
- Run `autolab doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `autolab config set`. Keep a backup of `~/.autolab/autolab.json`.
...
```bash
autolab gateway call config.get --params '{}' # capture payload.hash
autolab gateway call config.apply --params '{
...
```bash
autolab gateway call config.get --params '{}' # capture payload.hash
autolab gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.autolab/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@autolab", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.autolab/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.autolab/workspace" }],
}
```
...
{ "$include": "/etc/autolab/base.json5" } // absolute
...
```json5
// ~/.autolab/clients/mueller/agents.json5
[
...
```json5
// ~/.autolab/clients/mueller/broadcast.json5
{
...
AutoLab reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, AutoLab runs your login shell and imports only the missing expected keys (never overrides).
...
- `AUTOLAB_LOAD_SHELL_ENV=1`
- `AUTOLAB_SHELL_ENV_TIMEOUT_MS=15000`
...
AutoLab stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.autolab/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.autolab/credentials/oauth.json` (or `$AUTOLAB_STATE_DIR/credentials/oauth.json`)
...
- `~/.autolab/agent/*` (migrated by `autolab doctor` into `~/.autolab/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `AUTOLAB_OAUTH_DIR`
- Agent dir (default agent root override): `AUTOLAB_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, AutoLab imports `oauth.json` entries into `auth-profiles.json`.
...
If set, AutoLab derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/autolab/autolab-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/autolab/autolab.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.autolab/credentials/whatsapp/biz
          // authDir: "~/.autolab/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `autolab doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@autolab", "autolab"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@autolab"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.autolab/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.autolab/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.autolab/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.autolab/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.autolab/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.autolab/workspace-home" },
      { id: "work", workspace: "~/.autolab/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `autolab.json`).
...
AutoLab starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["autolab-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-autolab",
...
AutoLab starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `autolab plugins install @autolab/mattermost` (or `./extensions/mattermost` from a git checkout).
...
AutoLab starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
AutoLab spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, AutoLab
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/autolab" } },
}
```
...
When a file exceeds this limit, AutoLab logs a warning and injects a truncated
...
message envelopes). If unset, AutoLab uses the host timezone at runtime.
...
`[autolab]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[autolab]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[autolab]"`, AutoLab will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, AutoLab generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.autolab/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Labby"`).
...
AutoLab also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, AutoLab currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `autolab configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:autolab`: all built-in AutoLab tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.autolab/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.autolab/sandboxes",
        docker: {
          image: "autolab-sandbox:bookworm-slim",
          containerPrefix: "autolab-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "autolab-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "autolab-sandbox-browser:bookworm-slim",
          containerPrefix: "autolab-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
AutoLab uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.autolab/agents/<agentId>/agent/models.json` or by defining the same schema inside your
AutoLab config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, AutoLab writes/merges a `models.json` into
`~/.autolab/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. AutoLab uses
...
- Shortcut: `autolab onboard --auth-choice opencode-zen`.
...
Shortcut: `autolab onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `autolab onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `autolab onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `autolab onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `autolab onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `autolab onboard --auth-choice minimax-api`.
...
- Override the agent config root with `AUTOLAB_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.autolab/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.autolab/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.autolab/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, AutoLab stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.autolab/skills` (workspace skills
...
from `~/.autolab/extensions`, `<workspace>/.autolab/extensions`, plus any
...
### `browser` (autolab-managed browser)

AutoLab can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for autolab and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (AutoLab.app menubar, or `autolab gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      autolab: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/autolab" }
...
- Examples: `"/ui"`, `"/autolab"`, `"/apps/autolab"`.
...
- When a connection comes from one of these IPs, AutoLab uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `autolab gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `AUTOLAB_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `AUTOLAB_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. AutoLab
...
- AutoLab.app watches `~/.autolab/autolab.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.autolab/autolab.json` (or `AUTOLAB_CONFIG_PATH`) and applies changes automatically.
...
- `~/.autolab/autolab.json` (or `AUTOLAB_CONFIG_PATH`)
...
- `AUTOLAB_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `autolab --dev …` → uses `~/.autolab-dev` + shifts ports from base `19001`
- `autolab --profile <name> …` → uses `~/.autolab-<name>` (port via config/env/flags)
...
```bash
AUTOLAB_CONFIG_PATH=~/.autolab/a.json \
AUTOLAB_STATE_DIR=~/.autolab-a \
autolab gateway --port 19001
```
...
Gmail helper config (used by `autolab webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "autolab@gmail.com",
...
- Set `AUTOLAB_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, AutoLab defaults `serve.path` to `/` so
...
Default root: `~/.autolab/workspace/canvas`  
Default port: `18793` (chosen to avoid the autolab browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__autolab__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__autolab__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `AUTOLAB_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.autolab/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.autolab/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.autolab/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_autolab-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `autolab` (advertises `autolab.local`). Override with `AUTOLAB_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_autolab-gw._tcp` under `~/.autolab/dns/` using the configured discovery domain (example: `autolab.internal.`).
