...
read_when:
  - 安装 AutoLab
  - 你想从 GitHub 安装
summary: 安装 AutoLab（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 autolab、node-llama-cpp、sharp 等
pnpm add -g autolab@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm autolab ...` 运行仓库命令。
...
- 运行新手引导：`autolab onboard --install-daemon`
- 快速检查：`autolab doctor`
- 检查 Gateway 网关健康状态：`autolab status` + `autolab health`
- 打开仪表板：`autolab dashboard`
...
```bash
# 显式 npm
curl -fsSL https://dvallier-ai.github.io/autolab-public/install.sh | bash -s -- --install-method npm
...
- `AUTOLAB_GIT_DIR=...`
- `AUTOLAB_GIT_UPDATE=0|1`
- `AUTOLAB_NO_PROMPT=1`
- `AUTOLAB_DRY_RUN=1`
- `AUTOLAB_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `autolab`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `autolab`）。
