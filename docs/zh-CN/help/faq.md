...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [无法访问 docs.autolab.ai（SSL 错误），怎么办？](#i-cant-access-docsautolabai-ssl-error-what-now)
...
https://github.com/autolab/autolab/issues
https://github.com/autolab/autolab/pulls
...
查看 GitHub 变更日志：
https://github.com/autolab/autolab/blob/main/CHANGELOG.md

最新条目在顶部。如果顶部部分标记为 **Unreleased**，则下一个带日期的部分是最新发布版本。条目按**亮点**、**变更**和**修复**分组（需要时还有文档/其他部分）。

### 无法访问 docs.autolab.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.autolab.ai`。禁用该功能或将 `docs.autolab.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsautolabai-shows-an-ssl-error-comcastxfinity)。
请帮助我们在此处报告以解除封锁：https://spa.xfinity.com/check_url_status。

如果仍然无法访问该网站，文档在 GitHub 上有镜像：
https://github.com/autolab/autolab/tree/main/docs
...
展示：https://autolab.ai/showcase

## Skills 与自动化

### 如何自定义 Skills 而不弄脏仓库

使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.autolab/skills/<name>/SKILL.md`（或通过 `~/.autolab/autolab.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.autolab/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.autolab/autolab.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.autolab/skills` → 内置 → `skills.load.extraDirs`。`autohub` 默认安装到 `./skills`，AutoLab 将其视为 `<workspace>/skills`。
...
使用 **AutoHub**（CLI）或将 Skills 放入你的工作区。macOS Skills UI 在 Linux 上不可用。
浏览 Skills：https://autohub.com。

安装 AutoHub CLI（选择一个包管理器）：
...
AutoHub 安装到当前目录下的 `./skills`（或回退到你配置的 AutoLab 工作区）；AutoLab 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.autolab/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [AutoHub](/tools/autohub)。
...
AutoLab 从 `$AUTOLAB_CONFIG_PATH`（默认：`~/.autolab/autolab.json`）读取可选的 **JSON5** 配置：
...
- 从备份恢复（git 或复制的 `~/.autolab/autolab.json`）。
...
- 编辑 `~/.autolab/autolab.json` 中的 `agents.defaults.model`
...
仍然卡住？在 [Discord](https://discord.com/invite/autolab) 中提问或发起 [GitHub 讨论](https://github.com/autolab/autolab/discussions)。
