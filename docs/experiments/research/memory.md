---
summary: "Research notes: offline memory system for Labby workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.autolab/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AutoLab integration
...
Target: Labby-style workspace (`agents.defaults.workspace`, default `~/.autolab/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.autolab/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Labby to see every session.
...
```
~/.autolab/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in AutoLab**, but keep a separable core library.

### Why integrate into AutoLab?

- AutoLab already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `autolab memory recall "…" --k 25 --since 30d`
  - `autolab memory reflect --since 7d`
...
Pragmatic take for `~/.autolab/workspace`:
