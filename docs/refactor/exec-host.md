...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.autolab/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.autolab/node.json` (node id + pairing token).
...
Path: `~/.autolab/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.autolab/exec-approvals.sock",
...
- Unix socket at `~/.autolab/exec-approvals.sock` (0600).
