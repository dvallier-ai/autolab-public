...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AutoLab
...
Running AutoLab via Node with `tsx` fails at startup with:

```
[autolab] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `autolab status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch autolab.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node autolab.mjs status` works on Node 25.
