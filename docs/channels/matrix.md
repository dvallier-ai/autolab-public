...
Matrix is an open, decentralized messaging protocol. AutoLab connects as a Matrix **user**
...
```bash
autolab plugins install @autolab/matrix
```

Local checkout (when running from a git repo):

```bash
autolab plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `autolab plugins install @autolab/matrix`
   - From a local checkout: `autolab plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AutoLab calls the same
     login endpoint, stores the access token in `~/.autolab/credentials/matrix/credentials.json`,
...
- On first connection, AutoLab requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AutoLab logs a warning.
...
`~/.autolab/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `autolab pairing list matrix`
  - `autolab pairing approve matrix <CODE>`
...
- On startup, AutoLab resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
