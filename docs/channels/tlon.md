...
Tlon is a decentralized messenger built on Urbit. AutoLab connects to your Urbit ship and can
...
```bash
autolab plugins install @autolab/tlon
```

Local checkout (when running from a git repo):

```bash
autolab plugins install ./extensions/tlon
```
...
By default, AutoLab blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `autolab message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AutoLab replies in-thread.
