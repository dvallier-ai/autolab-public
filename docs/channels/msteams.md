...
```bash
autolab plugins install @autolab/msteams
```

Local checkout (when running from a git repo):

```bash
autolab plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
AutoLab will offer the local install path automatically.
...
3. Configure AutoLab with those credentials.
...
- Talk to AutoLab via Teams DMs, group chats, or channels.
...
- On startup, AutoLab resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring AutoLab, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `autolab plugins install @autolab/msteams`
   - From a local checkout: `autolab plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "AutoLab in Teams", "full": "AutoLab in Teams" },
...
AutoLab handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, AutoLab only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AutoLabShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

AutoLab sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `autolab message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.autolab/msteams-polls.json`.
...
```bash
autolab message send --channel msteams \
...
autolab message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
autolab message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
autolab message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
autolab message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
