...
- `/usage cost` in chats: local cost summary aggregated from AutoLab session logs.
- CLI: `autolab status --usage` prints a full per-provider breakdown.
- CLI: `autolab channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
