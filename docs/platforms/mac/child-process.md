...
service via the external `autolab` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`AUTOLAB_PROFILE`; legacy `com.autolab.*` is supported).
