...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `autolab sandbox explain`.
...
Legacy `agent.*` configs are migrated by `autolab doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=autolab-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${AUTOLAB_STATE_DIR:-$HOME/.autolab}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
