...
- **gateway host** → `autolab` process on the gateway machine
...
`~/.autolab/exec-approvals.json` directly.

CLI: `autolab approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
