---
summary: "Chrome extension: let AutoLab drive your existing Chrome tab"
read_when:
...
The AutoLab Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate autolab-managed Chrome profile.
...
AutoLab then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AutoLab release (npm package) as static files. There is no separate “build” step.

After upgrading AutoLab:

- Re-run `autolab browser extension install` to refresh the installed files under your AutoLab state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

AutoLab ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want AutoLab to control.
...
- `ON`: attached; AutoLab can drive that tab.
...
Debugging: `autolab sandbox explain`
...
`autolab browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `autolab browser extension install` first to copy the extension to a stable location under your AutoLab state directory.
...
- **This is not isolated** like the dedicated autolab-managed profile.
