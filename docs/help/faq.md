---
summary: "Frequently asked questions about AutoLab setup, configuration, and usage"
title: "FAQ"
---
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up AutoLab?](#whats-the-recommended-way-to-install-and-set-up-autolab)
  ...
  - [I can't access dvallier-ai.github.io/autolab-public (SSL error). What now?](#i-cant-access-docsautolabai-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or autolab not recognized](#windows-install-says-git-not-found-or-autolab-not-recognized)
  - [The docs didn't answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install AutoLab on Linux?](#how-do-i-install-autolab-on-linux)
  - [How do I install AutoLab on a VPS?](#how-do-i-install-autolab-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask AutoLab to update itself?](#can-i-ask-autolab-to-update-itself)
  ...
  - [If I buy a Mac mini to run AutoLab, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-autolab-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different AutoLab instances?](#can-multiple-people-use-one-whatsapp-number-with-different-autolab-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run AutoLab on a dedicated machine?](#how-important-is-it-to-run-autolab-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run AutoLab in a VM and what are the requirements](#can-i-run-autolab-in-a-vm-and-what-are-the-requirements)
- [What is AutoLab?](#what-is-autolab)
  - [What is AutoLab, in one paragraph?](#what-is-autolab-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for AutoLab](#what-are-the-top-five-everyday-use-cases-for-autolab)
  - [Can AutoLab help with lead gen outreach ads and blogs for a SaaS](#can-autolab-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can AutoLab run tasks on a schedule or continuously in the background?](#can-autolab-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with AutoLab saved locally?](#is-all-data-used-with-autolab-saved-locally)
  - [Where does AutoLab store its data?](#where-does-autolab-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall AutoLab?](#how-do-i-completely-uninstall-autolab)
...
- [Config basics](#config-basics)
  ...
  - [Can the AutoLab browser run headless?](#can-the-autolab-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two AutoLab instances talk to each other (local + VPS)?](#can-two-autolab-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does AutoLab load environment variables?](#how-does-autolab-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of AutoLab instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-autolab-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset AutoLab but keep it installed?](#how-do-i-completely-reset-autolab-but-keep-it-installed)
  ...
  - [Why doesn't AutoLab reply in a group?](#why-doesnt-autolab-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do AutoLab, Flawd, and Krill use for models?](#what-do-autolab-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `autolab gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-autolab-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `autolab gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-autolab-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run AutoLab in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-autolab-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart AutoLab?](#i-closed-my-terminal-on-windows-how-do-i-restart-autolab)
  ...
  - [ELI5: `autolab gateway restart` vs `autolab gateway`](#eli5-autolab-gateway-restart-vs-autolab-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose AutoLab to inbound DMs?](#is-it-safe-to-expose-autolab-to-inbound-dms)
...
7. **Gateway snapshot**
   ...
   ```bash
   autolab health --json
   autolab health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://dvallier-ai.github.io/autolab-public/install.sh | bash -s -- --install-method git
```

This installs AutoLab **from a git checkout**, so the agent can read the code + docs and
...
```bash
autolab status
autolab models status
autolab doctor
```

What they do:

- `autolab status`: quick snapshot of gateway/agent health + basic config.
- `autolab models status`: checks provider auth + model availability.
- `autolab doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `autolab status --all`, `autolab logs --follow`,
`autolab gateway status`, `autolab health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up AutoLab
...
If you don't have a global install yet, run it via `pnpm autolab onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `AUTOLAB_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `autolab config get gateway.auth.token` (or generate one: `autolab doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `autolab gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run `autolab gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install AutoLab on the new machine.
2. Copy `$AUTOLAB_STATE_DIR` (default: `~/.autolab`) from the old machine.
3. Copy your workspace (default: `~/.autolab/workspace`).
4. Run `autolab doctor` and restart the Gateway service.
...
under `~/.autolab/` (for example `~/.autolab/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-autolab-store-its-data),
...
### I cant access dvallier-ai.github.io/autolab-public SSL error What now

Some Comcast/Xfinity connections incorrectly block `dvallier-ai.github.io/autolab-public` via Xfinity
Advanced Security. Disable it or allowlist `dvallier-ai.github.io/autolab-public`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsautolabai-shows-an-ssl-error-comcastxfinity).
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://dvallier-ai.github.io/autolab-public/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://dvallier-ai.github.io/autolab-public/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://dvallier-ai.github.io/autolab-public/install.ps1](https://dvallier-ai.github.io/autolab-public/install.ps1)
...
```bash
curl -fsSL https://dvallier-ai.github.io/autolab-public/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://dvallier-ai.github.io/autolab-public/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://dvallier-ai.github.io/autolab-public/install.sh | bash -s -- --install-method git --verbose
```
...
### Windows install says git not found or autolab not recognized
...
**2) autolab is not recognized after install**
...
### How do I install AutoLab on Linux
...
### How do I install AutoLab on a VPS
...
### Can I ask AutoLab to update itself
...
```bash
autolab update
autolab update status
autolab update --channel stable|beta|dev
autolab update --tag <dist-tag|version>
autolab update --no-restart
```

If you must automate from an agent:

```bash
autolab update --yes --no-restart
autolab gateway restart
```
...
`autolab onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run AutoLab with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `autolab models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `autolab models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `autolab models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. AutoLab no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so AutoLab can keep replying while a provider is rate-limited.
...
AutoLab supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. AutoLab fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `autolab.json`.

Steps:

1. Enable the plugin: `autolab plugins enable google-gemini-cli-auth`
2. Login: `autolab models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. AutoLab needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. AutoLab runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run AutoLab can I connect it to my MacBook Pro
...
- Use `autolab nodes status` / `autolab nodes list` to see it.
...
- DM your bot, then run `autolab logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different AutoLab instances
...
If you run AutoLab via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the AutoLab code install. Your state
(`~/.autolab`) and workspace (`~/.autolab/workspace`) stay untouched.
...
**AutoLab-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run AutoLab on a dedicated machine
...
AutoLab is lightweight. For a basic Gateway + one chat channel:
...
### Can I run AutoLab in a VM and what are the requirements
...
### What is AutoLab in one paragraph

AutoLab is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

AutoLab is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for AutoLab
...
### Can AutoLab help with lead gen outreach ads and blogs for a SaaS
...
AutoLab draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

AutoLab is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use AutoLab when you
...
Showcase: [https://dvallier-ai.github.io/autolab-public/showcase](https://dvallier-ai.github.io/autolab-public/showcase)
...
- Confirm cron is enabled (`cron.enabled`) and `AUTOLAB_SKIP_CRON` is not set.
...
```bash
autolab cron run <jobId> --force
autolab cron runs --id <jobId> --limit 50
```
...
Not directly. macOS skills are gated by `metadata.autolab.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-autolab-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. AutoLab can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.autolab/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "autolab": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
- Persist `/home/node` with `AUTOLAB_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `AUTOLAB_DOCKER_APT_PACKAGES`.
...
AutoLab memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

AutoLab also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, AutoLab auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, AutoLab
prefers `local`.
...
### Is all data used with AutoLab saved locally

No - **AutoLab's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.autolab` + your workspace directory).
...
### Where does AutoLab store its data
...
| `$AUTOLAB_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
| `$AUTOLAB_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                        |
| `$AUTOLAB_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                     |
| `$AUTOLAB_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                 |

Legacy single-agent path: `~/.autolab/agent/*` (migrated by `autolab doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.autolab/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.autolab`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.autolab`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.autolab/skills`).

Default workspace is `~/.autolab/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.autolab/workspace" } },
}
```
...
Do **not** commit anything under `~/.autolab` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall AutoLab
...
`workspace` to the repo root. The AutoLab repo is just source code; keep the
...
If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.autolab/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `AUTOLAB_GATEWAY_TOKEN`).
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: `autolab doctor --generate-gateway-token`.
...
key. **Recommended:** run `autolab configure --section web` to store it in
...
- Daemons read env vars from `~/.autolab/.env` (or the service environment).
...
### Can the AutoLab browser run headless
...
5. Approve the node on the Gateway:
   ...
   ```bash
   autolab nodes pending
   autolab nodes approve <requestId>
   ```
...
- Gateway is running: `autolab gateway status`
- Gateway health: `autolab status`
- Channel health: `autolab channels status`
...
### Can two AutoLab instances talk to each other local VPS
...
`autolab agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
autolab agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Use `autolab config set` for small changes.
- Use `autolab configure` for interactive edits.
...
```bash
autolab gateway --tailscale serve
```
...
### How does AutoLab load environment variables

AutoLab reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.autolab/.env` (aka `$AUTOLAB_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.autolab/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`AUTOLAB_LOAD_SHELL_ENV=1`, `AUTOLAB_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`autolab models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means AutoLab won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.autolab/.env`:
...
### Is there a way to make a team of AutoLab instances one CEO and many agents
...
### How do I completely reset AutoLab but keep it installed
...
```bash
autolab reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `AUTOLAB_PROFILE`), reset each state dir (defaults are `~/.autolab-<profile>`).
- Dev reset: `autolab gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), AutoLab skips the heartbeat run to save API calls.
...
No. AutoLab runs on **your own account**, so if you're in the group, AutoLab can see it.
...
```bash
autolab directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt AutoLab reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.autolab/agents/<agentId>/sessions/`.
...
- Use `autolab doctor` to spot stray workspaces and profile mismatches.
...
AutoLab's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, AutoLab currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
### What do AutoLab, Flawd, and Krill use for models

- **AutoLab + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
...
Yes. AutoLab ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.autolab/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `autolab agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so AutoLab can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.autolab/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.autolab/agent/*` (migrated by `autolab doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.autolab/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `autolab models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `autolab models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `autolab models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.autolab/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    autolab models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), AutoLab will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: AutoLab now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
AutoLab uses provider-prefixed IDs like:
...
AutoLab may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `autolab models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
autolab models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
autolab models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
autolab models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
autolab models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key whats the difference

AutoLab supports both:
...
```
--port > AUTOLAB_GATEWAY_PORT > gateway.port > default 18789
```

### Why does autolab gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `autolab gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does autolab gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `AUTOLAB_STATE_DIR` mismatch).

Fix:

```bash
autolab gateway install --force
```
...
AutoLab enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `autolab gateway --port <port>`.

### How do I run AutoLab in remote mode client connects to a Gateway elsewhere
...
- `autolab gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `autolab.control.settings.v1`.

Fix:

- Fastest: `autolab dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `autolab doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `AUTOLAB_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- Still stuck? Run `autolab status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `AUTOLAB_CONFIG_PATH` (per-instance config)
- `AUTOLAB_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `autolab --profile <name> …` per instance (auto-creates `~/.autolab-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `autolab --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.autolab.*`, `autolab-gateway-<profile>.service`, `AutoLab Gateway (<profile>)`).
...
```
autolab tui --url ws://<host>:18789 --token <token>
```
...
- macOS: `$AUTOLAB_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.autolab/logs/...`; profiles use `~/.autolab-<profile>/logs/...`)
- Linux: `journalctl --user -u autolab-gateway[-<profile>].service -n 200 --no-pager`
...
If you run the gateway manually, `autolab gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart AutoLab
...
1. Is the Gateway running? `autolab gateway status`
2. Is the Gateway healthy? `autolab status`
3. Does the UI have the right token? `autolab dashboard`
...
```bash
autolab channels status
autolab channels logs --channel telegram
```
...
### ELI5 autolab gateway restart vs autolab gateway

- `autolab gateway restart`: restarts the **background service** (launchd/systemd).
- `autolab gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `autolab gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [AutoLab assistant setup](/start/autolab) and [Agent send](/tools/agent-send).

CLI sending:

```bash
autolab message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose AutoLab to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `autolab pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `autolab pairing list <channel>` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `autolab doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. AutoLab only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
autolab pairing approve whatsapp <code>
```

List pending requests:

```bash
autolab pairing list whatsapp
```
...
AutoLab blocks **cross-provider** messaging by default. If a tool call is bound
