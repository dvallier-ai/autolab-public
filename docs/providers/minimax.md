---
summary: "Use MiniMax M2.1 in AutoLab"
read_when:
  - You want MiniMax models in AutoLab
...
```bash
autolab plugins enable minimax-portal-auth  # skip if already loaded.
autolab gateway restart  # restart if gateway is already running
autolab onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/danv-intel/autolab/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `autolab.json`:
...
- Use `autolab models list` and `autolab models set minimax/MiniMax-M2.1` to switch.
...
- Running `autolab configure` and selecting **MiniMax M2.1**, or
