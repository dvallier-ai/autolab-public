---
summary: "Use Venice AI privacy-focused models in AutoLab"
read_when:
  - You want privacy-focused inference in AutoLab
...
```bash
autolab onboard --auth-choice venice-api-key
```
...
```bash
autolab chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, AutoLab shows all available Venice models. Pick based on your needs:
...
```bash
autolab models set venice/claude-opus-45
autolab models set venice/llama-3.3-70b
```

List all available models:

```bash
autolab models list | grep venice
```

## Configure via `autolab configure`

1. Run `autolab configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AutoLab automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
autolab chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
autolab chat --model venice/claude-opus-45

# Use uncensored model
autolab chat --model venice/venice-uncensored

# Use vision model with image
autolab chat --model venice/qwen3-vl-235b-a22b

# Use coding model
autolab chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `autolab models list` to see currently available models. Some models may be temporarily offline.
