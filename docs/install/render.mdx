...
Deploy AutoLab on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/danv-intel/autolab)
...
repository configures everything needed to run AutoLab:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: AUTOLAB_STATE_DIR
        value: /data/.autolab
      - key: AUTOLAB_WORKSPACE_DIR
        value: /data/workspace
      - key: AUTOLAB_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/autolab`.
...
If you use the original AutoLab repository, Render will not auto-deploy your AutoLab. To update it, run a manual Blueprint sync from the dashboard.
...
For AutoLab, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AutoLab host.
