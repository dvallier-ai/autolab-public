...
Deploy AutoLab on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/autolabbot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/autolab` — Control UI

## What you get

- Hosted AutoLab Gateway + Control UI
...
- `AUTOLAB_STATE_DIR=/data/.autolab` (recommended)
- `AUTOLAB_WORKSPACE_DIR=/data/workspace` (recommended)
- `AUTOLAB_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your AutoLab state + workspace so you can migrate to another host without losing config or memory.
