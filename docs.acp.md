...
This document describes how the AutoLab ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`autolab acp` exposes an ACP agent over stdio and forwards prompts to a running
AutoLab Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a AutoLab Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `autolab acp` over stdio.

Example config:

```bash
autolab config set gateway.remote.url wss://gateway-host:18789
autolab config set gateway.remote.token <token>
```

Example run:

```bash
autolab acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
autolab acp --session agent:main:main
autolab acp --session agent:design:main
autolab acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “AutoLab ACP” to start a thread.

## Execution Model

- ACP client spawns `autolab acp` and speaks ACP messages over stdio.
...
```bash
autolab acp --session agent:main:main
autolab acp --session-label "support inbox"
autolab acp --reset-session
```
...
`autolab acp` resolves the Gateway URL and auth from CLI flags or config:
