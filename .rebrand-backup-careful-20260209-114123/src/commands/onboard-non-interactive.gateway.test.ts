...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.AUTOLAB_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "autolab");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.AUTOLAB_STATE_DIR = stateDir;
    process.env.AUTOLAB_CONFIG_PATH = path.join(stateDir, "autolab.json");
