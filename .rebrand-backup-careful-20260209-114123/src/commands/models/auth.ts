...
import { readConfigFileSnapshot, type AutoLabConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: AutoLabConfig, model: string): AutoLabConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("autolab plugins install")}\`.`,
    );
  }
