...
export function triggerAutoLabRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.AUTOLAB_SYSTEMD_UNIT,
        process.env.AUTOLAB_PROFILE,
      );
  ...
  const label =
    process.env.AUTOLAB_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AUTOLAB_PROFILE);
