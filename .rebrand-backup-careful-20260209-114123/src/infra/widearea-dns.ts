...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AUTOLAB_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*autolab-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "autolab";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "autolab");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "autolab-gw");
  ...
  records.push(`_autolab-gw._tcp IN PTR ${instanceLabel}._autolab-gw._tcp`);
  records.push(`${instanceLabel}._autolab-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._autolab-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; autolab-content-hash: ${contentHash}\n${contentBody}`;
}
