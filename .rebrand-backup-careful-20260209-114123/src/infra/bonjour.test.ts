...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.AUTOLAB_MDNS_HOSTNAME = "test-host";
    process.env.AUTOLAB_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("autolab-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("autolab (AutoLab)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("autolab");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("autolab.local");
