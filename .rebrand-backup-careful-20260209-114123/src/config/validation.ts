...
import type { AutoLabConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AutoLabConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: AutoLabConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AutoLabSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AutoLabConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AutoLabConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as AutoLabConfig)),
    ),
  };
}
