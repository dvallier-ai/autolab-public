...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.AUTOLAB_GATEWAY_PORT).toBe("18789");
    expect(env.AUTOLAB_GATEWAY_TOKEN).toBe("secret");
    expect(env.AUTOLAB_SERVICE_MARKER).toBe("autolab");
    expect(env.AUTOLAB_SERVICE_KIND).toBe("gateway");
    expect(typeof env.AUTOLAB_SERVICE_VERSION).toBe("string");
    expect(env.AUTOLAB_SYSTEMD_UNIT).toBe("autolab-gateway.service");
    if (process.platform === "darwin") {
      expect(env.AUTOLAB_LAUNCHD_LABEL).toBe("ai.autolab.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", AUTOLAB_PROFILE: "work" },
      port: 18789,
    });
    expect(env.AUTOLAB_SYSTEMD_UNIT).toBe("autolab-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.AUTOLAB_LAUNCHD_LABEL).toBe("ai.autolab.work");
    }
  });
});
