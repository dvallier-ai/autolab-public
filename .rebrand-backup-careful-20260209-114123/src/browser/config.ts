import type { BrowserConfig, BrowserProfileConfig, AutoLabConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_AUTOLAB_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "autolab" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AUTOLAB_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AUTOLAB_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AutoLab browser profile (routing config), not a Chrome user profile.
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AutoLabConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AUTOLAB_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "autolab";
