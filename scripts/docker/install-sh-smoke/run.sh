...
INSTALL_URL="${AUTOLAB_INSTALL_URL:-https://autolab.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${AUTOLAB_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${AUTOLAB_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="autolab"
PACKAGE_NAME="${AUTOLAB_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (AUTOLAB_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${AUTOLAB_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${AUTOLAB_INSTALL_LATEST_OUT:-}"
fi
