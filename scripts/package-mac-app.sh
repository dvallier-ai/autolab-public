...
# Build and bundle AutoLab into a minimal .app we can open.
# Outputs to dist/AutoLab.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AutoLab.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AutoLab"
BUNDLE_ID="${BUNDLE_ID:-ai.autolab.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/danv-intel/autolab/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AutoLab/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AutoLabBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AutoLabGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AutoLab"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AutoLab"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AutoLab"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AutoLab" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AutoLab/Resources/AutoLab.icns" "$APP_ROOT/Contents/Resources/AutoLab.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AutoLab/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AutoLabKit resources"
AUTOLABKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AutoLabKit_AutoLabKit.bundle"
if [ -d "$AUTOLABKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AutoLabKit_AutoLabKit.bundle"
  cp -R "$AUTOLABKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AutoLabKit_AutoLabKit.bundle"
else
  echo "WARN: AutoLabKit resource bundle not found at $AUTOLABKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AutoLab"
killall -q AutoLab 2>/dev/null || true
