...
BASE_IMAGE="${BASE_IMAGE:-autolab-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-autolab-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=autolab.sandbox=1)
NOTE
