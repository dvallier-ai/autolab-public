#!/usr/bin/env bash
# ClawDock - Docker helpers for AutoLab
# Inspired by Simon Willison's "Running AutoLab in Docker"
# https://til.simonwillison.net/llms/autolab-docker
#
# Installation:
...
# Common paths to check for AutoLab
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/autolab"
  "${HOME}/workspace/autolab"
  "${HOME}/projects/autolab"
  "${HOME}/dev/autolab"
  "${HOME}/code/autolab"
  "${HOME}/src/autolab"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found AutoLab at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/autolab"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ AutoLab not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/danv-intel/autolab.git ~/autolab"
    echo "  cd ~/autolab && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^AUTOLAB_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d autolab-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart autolab-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f autolab-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec autolab-gateway \
    bash -c 'echo "alias autolab=\"./autolab.mjs\"" > /tmp/.bashrc_autolab && bash --rcfile /tmp/.bashrc_autolab'
}
...
clawdock-cli() {
  _clawdock_compose run --rm autolab-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build autolab-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "AUTOLAB_GATEWAY_TOKEN=$token" autolab-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" autolab-gateway \
    bash -c './autolab.mjs config set gateway.remote.token "$TOKEN" && ./autolab.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec autolab-gateway \
    bash -c "./autolab.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart autolab-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm autolab-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec autolab-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'autolab config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for AutoLab${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (autolab alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to autolab project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.autolab)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'autolab channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'autolab status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.autolab.ai${_CLR_RESET}"
  echo ""
}
