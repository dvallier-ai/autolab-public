...
IMAGE_NAME="autolab-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="autolab-net-e2e-$$"
GW_NAME="autolab-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "AUTOLAB_GATEWAY_TOKEN=$TOKEN" \
	  -e "AUTOLAB_SKIP_CHANNELS=1" \
	  -e "AUTOLAB_SKIP_GMAIL_WATCHER=1" \
	  -e "AUTOLAB_SKIP_CRON=1" \
	  -e "AUTOLAB_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
