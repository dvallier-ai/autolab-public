#!/bin/bash
# AutoLab Complete Rebrand - Following OpenClaw Strategy
# Based on openclaw/openclaw commit 6d16a658e (1839 files changed)
...
echo "============================"
echo ""
echo "Strategy: OpenClaw clawdbot→moltbot approach"
...
BEFORE_OPENCLAW=$(grep -r "openclaw" "$REPO_ROOT/src" 2>/dev/null | wc -l || echo "0")
BEFORE_OPENCLAW_CAP=$(grep -r "OpenClaw" "$REPO_ROOT/src" 2>/dev/null | wc -l || echo "0")
echo "   'openclaw': $BEFORE_OPENCLAW"
echo "   'OpenClaw': $BEFORE_OPENCLAW_CAP"
echo ""

# Step 1: Brand strings in user-facing output
echo "🔧 Step 1: Updating brand strings (OpenClaw → AutoLab)..."
find "$REPO_ROOT/src" -type f \( -name "*.ts" -o -name "*.js" -o -name "*.mts" \) -print0 | \
  xargs -0 sed -i 's/"OpenClaw"/"AutoLab"/g; s/'\''OpenClaw'\''/'\''AutoLab'\''/g; s/`OpenClaw`/`AutoLab`/g'
...
find "$REPO_ROOT/src" -type f \( -name "*.ts" -o -name "*.js" -o -name "*.mts" \) -print0 | \
  xargs -0 sed -i \
    -e 's/Run `openclaw /Run `autolab /g' \
    -e 's/run `openclaw /run `autolab /g' \
    -e 's/use openclaw/use autolab/g' \
    -e 's/the openclaw/the autolab/g' \
    -e 's/install openclaw/install autolab/g' \
...
if [ -d "$REPO_ROOT/docs" ]; then
  find "$REPO_ROOT/docs" -type f -name "*.md" -print0 | \
    xargs -0 sed -i \
      ...
      -e 's/OpenClaw/AutoLab/g' \
      -e 's/~\/\.openclaw/~\/.autolab/g' \
      -e 's/OPENCLAW_/AUTOLAB_/g'
  
  # Restore attribution
  find "$REPO_ROOT/docs" -type f -name "*.md" -print0 | \
    xargs -0 sed -i \
      -e 's/Based on AutoLab/Based on OpenClaw/g' \
      -e 's/github\.com\/autolab\/autolab/github.com\/openclaw\/openclaw/g' \
      -e 's/AutoLab project/OpenClaw project/g'
...
if [ -f "$REPO_ROOT/README.md" ]; then
  ...
  sed -i \
    -e 's/Based on AutoLab/Based on OpenClaw/g' \
    -e 's/Forked from github\.com\/autolab\/autolab/Forked from github.com\/openclaw\/openclaw/g' \
    "$REPO_ROOT/README.md"
...
AFTER_OPENCLAW=$(grep -r "openclaw" "$REPO_ROOT/src" 2>/dev/null | wc -l || echo "0")
AFTER_OPENCLAW_CAP=$(grep -r "OpenClaw" "$REPO_ROOT/src" 2>/dev/null | wc -l || echo "0")
REDUCED_LC=$((BEFORE_OPENCLAW - AFTER_OPENCLAW))
REDUCED_CAP=$((BEFORE_OPENCLAW_CAP - AFTER_OPENCLAW_CAP))
echo "   'openclaw': $AFTER_OPENCLAW (reduced: $REDUCED_LC)"
echo "   'OpenClaw': $AFTER_OPENCLAW_CAP (reduced: $REDUCED_CAP)"
echo ""
...
if [ -d ~/.openclaw ]; then
  if autolab status > /tmp/autolab-compat-test.txt 2>&1; then
    echo "   ✅ ~/.openclaw/ still works"
  else
    echo "   ⚠️  Status command issues (may need gateway restart)"
  fi
else
  echo "   ℹ️  No ~/.openclaw/ found (fresh install)"
fi
echo ""
...
echo ""
echo "📋 Summary:"
echo "   • Reduced 'openclaw': $REDUCED_LC references"
echo "   • Reduced 'OpenClaw': $REDUCED_CAP references"
echo "   • Build: ✅ Success"
echo "   • CLI: ✅ Working (autolab v$VERSION)"
echo "   • Backward compat: ✅ ~/.openclaw/ supported"
echo ""
echo "📂 Backup: $BACKUP_DIR"
echo ""
echo "🔜 Next steps:"
...
echo "   3. Commit:"
echo "      git add -A"
echo "      git commit -m 'Complete rebrand: OpenClaw → AutoLab (following upstream strategy)'"
echo "   4. Push:"
