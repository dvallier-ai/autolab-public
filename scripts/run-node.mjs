...
const shouldBuild = (deps) => {
  if (deps.env.AUTOLAB_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.AUTOLAB_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[autolab] ${message}\n`);
};

const runAutoLab = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["autolab.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
