...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/autolab/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AutoLab Re-Auth" -c "After getting key, run: ssh $SERVER '~/autolab/scripts/mobile-reauth.sh'" --id autolab-auth
        ;;
    *)
