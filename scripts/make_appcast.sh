...
ZIP=${1:?"Usage: $0 AutoLab-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/danv-intel/autolab/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^AutoLab-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/danv-intel/autolab/releases/download/v${VERSION}/"}
