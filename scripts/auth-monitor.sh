...
#
# Suggested cron: */30 * * * * /home/admin/autolab/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AutoLab notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., autolab-alerts)
...
STATE_FILE="$HOME/.autolab/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AutoLab if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use autolab
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AutoLab to $NOTIFY_PHONE..."
            autolab send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AutoLab is down. Run: ssh l36 '~/autolab/scripts/mobile-reauth.sh'" "urgent"
    exit 1
