#!/bin/bash
# Setup AutoLab Auth Management System
...
echo "=== AutoLab Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'autolab-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AutoLab message: Send warning via AutoLab itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/autolab-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/autolab-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/autolab-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now autolab-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status autolab-auth-monitor.timer"
echo ""
echo "Quick commands:"
