...
# AutoLab Auth Widget for Termux
...
SERVER="${AUTOLAB_SERVER:-${CLAWDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking AutoLab auth..."

STATUS=$(ssh "$SERVER" '$HOME/autolab/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/autolab/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|AUTOLAB_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/autolab/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|AUTOLAB_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: AutoLab won't work until re-auth"
                ;;
        esac
        ;;
