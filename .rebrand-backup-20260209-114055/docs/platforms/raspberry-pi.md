---
summary: "AutoLab on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AutoLab on a Raspberry Pi
  - Running AutoLab on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# AutoLab on Raspberry Pi

## Goal

Run a persistent, always-on AutoLab Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most AutoLab features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled autolab

# Enable if not
sudo systemctl enable autolab

# Start on boot
sudo systemctl start autolab
```
...
```bash
# Check logs
journalctl -u autolab --no-pager -n 100

# Common fix: rebuild
cd ~/autolab  # if using hackable install
npm run build
sudo systemctl restart autolab
```
