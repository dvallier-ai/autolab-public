...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/autolab`)
...
If the page fails to load, start the Gateway first: `autolab gateway`.
...
you revoke it with `autolab devices revoke --device <id> --role <role>`. See
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AutoLab
...
AutoLab **blocks** Control UI connections without device identity.
