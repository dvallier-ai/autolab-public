...
AutoLab integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AutoLab owns lifecycle when `channels.signal.autoStart=true`.
...
- AutoLab spawns `imsg rpc` as a child process (legacy iMessage integration).
