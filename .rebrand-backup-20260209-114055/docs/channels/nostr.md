...
read_when:
  - You want AutoLab to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables AutoLab to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`autolab onboard`) and `autolab channels add` list optional channel plugins.
...
```bash
autolab plugins install @autolab/nostr
```

Use a local checkout (dev workflows):

```bash
autolab plugins install --link <path-to-autolab>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "autolab@example.com",
        "lud16": "autolab@example.com"
      }
    }
  }
}
