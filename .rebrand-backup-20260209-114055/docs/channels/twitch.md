...
read_when:
  - Setting up Twitch chat integration for AutoLab
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AutoLab connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
autolab plugins install @autolab/twitch
```

Local checkout (when running from a git repo):

```bash
autolab plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `AUTOLAB_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "autolab", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use AUTOLAB_TWITCH_ACCESS_TOKEN env var)
...
```bash
AUTOLAB_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
