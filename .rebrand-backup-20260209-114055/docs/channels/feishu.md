...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AutoLab to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
autolab plugins install @autolab/feishu
```

Local checkout (when running from a git repo):

```bash
autolab plugins install ./extensions/feishu
```
...
If you just installed AutoLab, run the wizard:
...
2. Configuring app credentials in AutoLab
...
1. You already ran `autolab channels add` for Feishu
2. The gateway is running (`autolab gateway status`)
...
```bash
autolab pairing approve feishu <CODE>
```
...
2. Run `autolab logs --follow` and look for `chat_id`
...
2. Run `autolab logs --follow` and look for `open_id`
...
| `autolab gateway restart` | Restart gateway service       |
| `autolab logs --follow`   | Tail gateway logs             |
...
4. Check logs: `autolab logs --follow`
...
5. Ensure the gateway is running: `autolab gateway status`
6. Check logs: `autolab logs --follow`
