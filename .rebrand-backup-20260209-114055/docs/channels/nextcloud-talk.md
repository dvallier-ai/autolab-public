...
```bash
autolab plugins install @autolab/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
autolab plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "AutoLab" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `autolab pairing list nextcloud-talk`
  - `autolab pairing approve nextcloud-talk <CODE>`
