...
- Publish as `autolab/plugin-sdk` (or export from core under `autolab/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AutoLabPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AutoLabConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AutoLabConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AutoLabConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AutoLabConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AutoLabConfig): string;
  };
};
```
...
- Introduce `autolab/plugin-sdk`.
- Add `api.runtime` to `AutoLabPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `autolabRuntime: ">=2026.2.0"`).
