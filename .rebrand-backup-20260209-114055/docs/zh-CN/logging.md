...
- AutoLab 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `autolab plugins enable diagnostics-otel` 启用插件。
...
- `autolab.tokens`（计数器，属性：`autolab.token`、`autolab.channel`、`autolab.provider`、`autolab.model`）
- `autolab.cost.usd`（计数器，属性：`autolab.channel`、`autolab.provider`、`autolab.model`）
- `autolab.run.duration_ms`（直方图，属性：`autolab.channel`、`autolab.provider`、`autolab.model`）
- `autolab.context.tokens`（直方图，属性：`autolab.context`、`autolab.channel`、`autolab.provider`、`autolab.model`）

消息流：

- `autolab.webhook.received`（计数器，属性：`autolab.channel`、`autolab.webhook`）
- `autolab.webhook.error`（计数器，属性：`autolab.channel`、`autolab.webhook`）
- `autolab.webhook.duration_ms`（直方图，属性：`autolab.channel`、`autolab.webhook`）
- `autolab.message.queued`（计数器，属性：`autolab.channel`、`autolab.source`）
- `autolab.message.processed`（计数器，属性：`autolab.channel`、`autolab.outcome`）
- `autolab.message.duration_ms`（直方图，属性：`autolab.channel`、`autolab.outcome`）

队列 + 会话：

- `autolab.queue.lane.enqueue`（计数器，属性：`autolab.lane`）
- `autolab.queue.lane.dequeue`（计数器，属性：`autolab.lane`）
- `autolab.queue.depth`（直方图，属性：`autolab.lane` 或 `autolab.channel=heartbeat`）
- `autolab.queue.wait_ms`（直方图，属性：`autolab.lane`）
- `autolab.session.state`（计数器，属性：`autolab.state`、`autolab.reason`）
- `autolab.session.stuck`（计数器，属性：`autolab.state`）
- `autolab.session.stuck_age_ms`（直方图，属性：`autolab.state`）
- `autolab.run.attempt`（计数器，属性：`autolab.attempt`）
...
- `autolab.webhook.error`
  - `autolab.channel`、`autolab.webhook`、`autolab.chatId`、`autolab.error`
- `autolab.message.processed`
  - `autolab.channel`、`autolab.outcome`、`autolab.chatId`、`autolab.messageId`、`autolab.sessionKey`、`autolab.sessionId`、`autolab.reason`
- `autolab.session.stuck`
  - `autolab.state`、`autolab.ageMs`、`autolab.queueDepth`、`autolab.sessionKey`、`autolab.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `autolab doctor`。
