...
const requireAgentDir = () => {
  const agentDir = process.env.AUTOLAB_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AUTOLAB_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.AUTOLAB_OAUTH_DIR;
  });

  it("writes auth-profiles.json under AUTOLAB_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-oauth-"));
    process.env.AUTOLAB_STATE_DIR = tempStateDir;
    process.env.AUTOLAB_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AUTOLAB_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AUTOLAB_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-minimax-"));
    process.env.AUTOLAB_STATE_DIR = tempStateDir;
    process.env.AUTOLAB_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AUTOLAB_AGENT_DIR;
