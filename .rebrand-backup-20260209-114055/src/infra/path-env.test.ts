...
import { ensureAutoLabCliOnPath } from "./path-env.js";

describe("ensureAutoLabCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling autolab exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-path-"));
    try {
      ...
      const originalFlag = process.env.AUTOLAB_PATH_BOOTSTRAPPED;
      ...
      try {
        ...
        if (originalFlag === undefined) {
          ...
          process.env.AUTOLAB_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
