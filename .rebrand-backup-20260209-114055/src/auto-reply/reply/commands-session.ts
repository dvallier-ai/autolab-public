...
import { scheduleGatewaySigusr1Restart, triggerAutoLabRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting AutoLab in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAutoLabRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting AutoLab via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
