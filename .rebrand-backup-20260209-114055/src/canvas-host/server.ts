...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.autolabCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.autolabCanvasA2UIAction &&
        typeof window.autolabCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.autolabSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("autolab:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AutoLab node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.autolabSendUserAction === "function"
        ? window.autolabSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AUTOLAB_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.AUTOLAB_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AutoLab Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
