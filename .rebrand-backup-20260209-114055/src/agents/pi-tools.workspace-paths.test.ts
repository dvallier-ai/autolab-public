...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("autolab-ws-", async (workspaceDir) => {
      await withTempDir("autolab-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAutoLabCodingTools({ workspaceDir });
    ...
    await withTempDir("autolab-ws-", async (workspaceDir) => {
      await withTempDir("autolab-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello autolab");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("autolab-ws-", async (workspaceDir) => {
      const tools = createAutoLabCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("autolab-ws-", async (workspaceDir) => {
      await withTempDir("autolab-override-", async (overrideDir) => {
        const tools = createAutoLabCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("autolab-sandbox-", async (sandboxDir) => {
      await withTempDir("autolab-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "autolab-sbx-test",
        ...
        const tools = createAutoLabCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
