...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.AUTOLAB_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-test-"));
    process.env.AUTOLAB_STATE_DIR = stateDir;
    process.env.AUTOLAB_PROFILE = "isolated";

    try {
      const tool = createAutoLabTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: autolab --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/autolab" } }\n}\n';
