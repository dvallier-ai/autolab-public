...
const execHostEnforced = process.env.AUTOLAB_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.AUTOLAB_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureAutoLabCliOnPath({ pathEnv: process.env.PATH ?? "" });
