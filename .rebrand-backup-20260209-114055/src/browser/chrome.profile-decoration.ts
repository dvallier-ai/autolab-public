...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".autolab-profile-decorated");
}
 ...
 */
export function decorateAutoLabProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AUTOLAB_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AUTOLAB_BROWSER_COLOR).toUpperCase();
