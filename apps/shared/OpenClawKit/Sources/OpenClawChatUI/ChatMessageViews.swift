...
struct ChatMessageBubble: View {
    let message: AutoLabChatMessage
    let style: AutoLabChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AutoLabChatTheme.userText : AutoLabChatTheme.assistantText
    ...
    private var inlineAttachments: [AutoLabChatMessageContent] {
    ...
    private var toolCalls: [AutoLabChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AutoLabChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AutoLabChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AutoLabChatTheme.onboardingAssistantBubble
        }
        return AutoLabChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AutoLabChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AutoLabChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AutoLabChatTheme.userText : AutoLabChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AutoLabChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AutoLabChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AutoLabChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AutoLabChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AutoLabChatTheme.assistantText)
            }
        }
    }
}
