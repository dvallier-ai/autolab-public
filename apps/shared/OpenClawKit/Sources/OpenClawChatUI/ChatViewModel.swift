...
private let chatUILogger = Logger(subsystem: "ai.autolab", category: "AutoLabChatUI")

@MainActor
@Observable
public final class AutoLabChatViewModel {
    public private(set) var messages: [AutoLabChatMessage] = []
    ...
    public var attachments: [AutoLabPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AutoLabChatPendingToolCall] = []
    public private(set) var sessions: [AutoLabChatSessionEntry] = []
    private let transport: any AutoLabChatTransport
    ...
    private var pendingToolCallsById: [String: AutoLabChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AutoLabChatTransport) {
    ...
    public var sessionChoices: [AutoLabChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [AutoLabChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [AutoLabChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AutoLabPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AutoLabChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AutoLabChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [AutoLabChatMessage]) -> [AutoLabChatMessage] {
        var result: [AutoLabChatMessage] = []
    ...
    private static func dedupeKey(for message: AutoLabChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AutoLabChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> AutoLabChatAttachmentPayload in
            AutoLabChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AutoLabChatSessionEntry {
        AutoLabChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AutoLabChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AutoLabChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AutoLabAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AutoLabChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AutoLabPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
