...
@MainActor
struct AutoLabChatComposer: View {
    @Bindable var viewModel: AutoLabChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AutoLabChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AutoLabChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AutoLabPendingAttachment.id)
                { (att: AutoLabPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AutoLabPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AutoLabChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AutoLabChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AutoLabChatTheme.subtleCard)
        .clipShape(Capsule())
    }
