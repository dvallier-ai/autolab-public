...
struct AutoLabMacCLI {
    static func main() async {
        ...
        default:
            fputs("autolab-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      autolab-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      autolab-mac discover [--timeout <ms>] [--json] [--include-local]
      autolab-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      autolab-mac connect
      autolab-mac connect --url ws://127.0.0.1:18789 --json
      autolab-mac discover --timeout 3000 --json
      autolab-mac wizard --mode local
    """)
}
