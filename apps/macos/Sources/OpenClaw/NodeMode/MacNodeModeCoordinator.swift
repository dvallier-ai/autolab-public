...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.autolab", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: AutoLabNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [AutoLabCapability.canvas.rawValue, AutoLabCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(AutoLabCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if AutoLabLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(AutoLabCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            AutoLabSystemCommand.which.rawValue,
            AutoLabSystemCommand.run.rawValue,
            AutoLabSystemCommand.execApprovalsGet.rawValue,
            AutoLabSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(AutoLabCapability.camera.rawValue) {
        ...
        if capsSet.contains(AutoLabCapability.location.rawValue) {
