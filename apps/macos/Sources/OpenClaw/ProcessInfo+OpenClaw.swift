...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.autolab.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["AUTOLAB_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "autolab.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "autolab.nixMode") { return true }
