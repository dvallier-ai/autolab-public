...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: AutoLabOAuthStore.AnthropicOAuthStatus = AutoLabOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: AutoLabOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = AutoLabOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = AutoLabOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try AutoLabOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. AutoLab can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: AutoLabOAuthStore.AnthropicOAuthStatus,
