...
enum DebugActions {
    private static let verboseDefaultsKey = "autolab.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "autolab.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AutoLabPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AutoLab", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AutoLabPaths.configURL
        guard
