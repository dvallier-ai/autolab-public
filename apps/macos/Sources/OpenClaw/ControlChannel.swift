...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AutoLabProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.autolab", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AutoLabKit.AnyCodable]()) {
                $0[$1.key] = AutoLabKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AutoLabProtocol.AnyCodable?) -> [String: AutoLabProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AutoLabProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AutoLabKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AutoLabProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("autolab.control.heartbeat")
    static let controlAgentEvent = Notification.Name("autolab.control.agent")
}
