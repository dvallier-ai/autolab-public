...
private let webChatSwiftLogger = Logger(subsystem: "ai.autolab", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AutoLabChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AutoLabChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AutoLabChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AutoLabChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AutoLabGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AutoLabChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AutoLabChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AutoLabChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AutoLabChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AutoLabChatView>) -> NSViewController
    {
