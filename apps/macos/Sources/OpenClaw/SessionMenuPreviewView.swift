...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.autolab", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AutoLabSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AutoLabSessionPreviewEntry,
    ...
    private static func cache(payload: AutoLabSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AutoLabChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AutoLabKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AutoLabKit.AnyCodable]) -> [AutoLabChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AutoLabChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AutoLabChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AutoLabChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AutoLabChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AutoLabChatMessage) -> String? {
