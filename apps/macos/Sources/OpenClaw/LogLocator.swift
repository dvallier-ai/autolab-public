...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AUTOLAB_LOG_DIR"],
           !override.isEmpty
        {
    ...
    private static var stdoutLog: URL {
        logDir.appendingPathComponent("autolab-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("autolab-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/autolab/ (rolling or stdout), or nil if none exist.
