...
private let deepLinkLogger = Logger(subsystem: "ai.autolab", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "AutoLab is paused", message: "Unpause AutoLab to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run AutoLab agent?", message: body) else { return }
        }
