...
enum CommandResolver {
    private static let projectRootDefaultsKey = "autolab.gatewayProjectRootPath"
    private static let helperName = "autolab"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let autolabEntry = root.appendingPathComponent("autolab.mjs").path
        if FileManager().isReadableFile(atPath: autolabEntry) { return autolabEntry }
        let binEntry = root.appendingPathComponent("bin/autolab.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/autolab")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let autolabPaths = self.autolabManagedPaths(home: home)
        if !autolabPaths.isEmpty {
            extras.insert(contentsOf: autolabPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + autolabPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func autolabManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".autolab"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func autolabExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAutoLabExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("autolab.mjs").path,
            root.appendingPathComponent("bin/autolab.js").path,
        ]
    ...
    static func hasAnyAutoLabInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.autolabExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func autolabNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let autolabPath = self.projectAutoLabExecutable(projectRoot: root) {
                return [autolabPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "autolab", subcommand] + extraArgs
            }
            if let autolabPath = self.autolabExecutable(searchPaths: searchPaths) {
                return [autolabPath, subcommand] + extraArgs
            }

            let missingEntry = """
            autolab entrypoint missing (looked for dist/index.js or autolab.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real autolab CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/autolab"
        ...
        \(cliSection)
        \(projectSection)
        if command -v autolab >/dev/null 2>&1; then
          CLI="$(command -v autolab)"
          autolab \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/autolab.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/autolab.mjs"
            node "$PRJ/autolab.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/autolab.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/autolab.js"
            node "$PRJ/bin/autolab.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent autolab"
          pnpm --silent autolab \(quotedArgs);
        else
          echo "autolab CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AutoLabConfigFile.loadDict()
