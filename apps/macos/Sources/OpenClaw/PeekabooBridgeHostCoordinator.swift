...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.autolab", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: AutoLabPeekabooBridgeServices?
    private static var autolabSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("AutoLab", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = AutoLabPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.autolabSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.autolabSocketPath, privacy: .public)")
    }
...
@MainActor
private final class AutoLabPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.autolab.peekaboo")
