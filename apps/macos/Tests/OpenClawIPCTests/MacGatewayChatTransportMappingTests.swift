...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: AutoLabProtocol.AnyCodable(["ok": AutoLabProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AutoLabProtocol.AnyCodable(["ok": AutoLabProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = AutoLabProtocol.AnyCodable([
            "runId": AutoLabProtocol.AnyCodable("run-1"),
            "sessionKey": AutoLabProtocol.AnyCodable("main"),
            "state": AutoLabProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AutoLabProtocol.AnyCodable(["a": AutoLabProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
