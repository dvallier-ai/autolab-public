...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: AutoLabProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AutoLabProtocol.AnyCodable] = [
            "presence": .init([AutoLabProtocol.AnyCodable(entry)]),
        ]
        let payload = AutoLabProtocol.AnyCodable(payloadMap)
