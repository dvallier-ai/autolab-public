...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> AutoLabNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> AutoLabNetworkStatusPayload {
        let status: AutoLabNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> AutoLabNetworkStatusPayload {
        AutoLabNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
