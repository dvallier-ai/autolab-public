...
struct IOSGatewayChatTransport: AutoLabChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AutoLabChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AutoLabChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AutoLabChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AutoLabChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AutoLabChatAttachmentPayload]) async throws -> AutoLabChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AutoLabChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(AutoLabChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(AutoLabGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AutoLabGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AutoLabChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AutoLabAgentEventPayload.self)
                        {
