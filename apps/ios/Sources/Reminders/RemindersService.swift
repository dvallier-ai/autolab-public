...
final class RemindersService: RemindersServicing {
    func list(params: AutoLabRemindersListParams) async throws -> AutoLabRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AutoLabReminderPayload], Error>) in
        ...
        return AutoLabRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AutoLabRemindersAddParams) async throws -> AutoLabRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AutoLabReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AutoLabRemindersAddPayload(reminder: payload)
    }
