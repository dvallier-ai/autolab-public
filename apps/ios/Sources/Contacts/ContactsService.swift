...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AutoLabContactsSearchParams) async throws -> AutoLabContactsSearchPayload {
        ...
        return AutoLabContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AutoLabContactsAddParams) async throws -> AutoLabContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AutoLabContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AutoLabContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AutoLabContactPayload {
        AutoLabContactPayload(
