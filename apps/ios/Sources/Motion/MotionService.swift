...
final class MotionService: MotionServicing {
    func activities(params: AutoLabMotionActivityParams) async throws -> AutoLabMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AutoLabMotionActivityEntry], Error>) in
        ...
        return AutoLabMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AutoLabPedometerParams) async throws -> AutoLabPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<AutoLabPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AutoLabPedometerPayload(
