...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = AutoLabLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.autolab.ios", category: "GatewaySettings")
            ...
            Form {
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        Text("Keeps the screen awake while AutoLab is open.")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
    ...
    private var locationMode: AutoLabLocationMode {
        AutoLabLocationMode(rawValue: self.locationEnabledModeRaw) ?? .off
    }
