...
This is an **alpha** iOS app that connects to an AutoLab Gateway as a `role: node`.
...
3. On iOS: AutoLab → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project AutoLab.xcodeproj -scheme AutoLab -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/AutoLabKit` contains the shared transport/types used by the iOS app.
