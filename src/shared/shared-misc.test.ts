...
describe("shared/frontmatter", () => {
  ...
  test("resolveAutoLabManifestBlock parses JSON5 metadata and picks autolab block", () => {
    const frontmatter = {
      metadata: "{ autolab: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveAutoLabManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveAutoLabManifestBlock returns undefined for invalid input", () => {
    expect(resolveAutoLabManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveAutoLabManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveAutoLabManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
