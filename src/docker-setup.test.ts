...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    AUTOLAB_CONFIG_DIR: join(sandbox.rootDir, "config"),
    AUTOLAB_WORKSPACE_DIR: join(sandbox.rootDir, "autolab"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        ...
        AUTOLAB_EXTRA_MOUNTS: undefined,
        AUTOLAB_HOME_VOLUME: "autolab-home",
      }),
    ...
    expect(envFile).toContain("AUTOLAB_HOME_VOLUME=autolab-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("autolab-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("autolab-home:");
