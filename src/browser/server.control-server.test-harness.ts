...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/autolab" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          autolab: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  resolveAutoLabUserDataDir: vi.fn(() => chromeUserDataDir.dir),
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.AUTOLAB_GATEWAY_PORT;
    process.env.AUTOLAB_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.AUTOLAB_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.AUTOLAB_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.AUTOLAB_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.AUTOLAB_GATEWAY_TOKEN;
    } else {
      process.env.AUTOLAB_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.AUTOLAB_GATEWAY_PASSWORD;
    } else {
      process.env.AUTOLAB_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
