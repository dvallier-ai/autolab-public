...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAutoLabTmpDir: vi.fn(() => "/tmp/autolab"),
}));
vi.mock("../infra/tmp-autolab-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAutoLabTmpDir.mockReturnValue("/tmp/autolab");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredAutoLabTmpDir.mockReturnValue("/tmp/autolab-preferred");
    setPwToolsCoreCurrentPage({ on, off });
    ...
    const expectedDownloadsTail = `${path.join("tmp", "autolab-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAutoLabTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "autolab-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "autolab-preferred", "downloads")}${path.sep}`),
    );
  });
