...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "autolab-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.autolab when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "autolab-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers AUTOLAB_HOME over HOME", () => {
    vi.stubEnv("AUTOLAB_HOME", "/srv/autolab-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/autolab-home"));
...
describe("shortenHomePath", () => {
  it("uses $AUTOLAB_HOME prefix when AUTOLAB_HOME is set", () => {
    vi.stubEnv("AUTOLAB_HOME", "/srv/autolab-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("shortenHomeInString", () => {
  it("uses $AUTOLAB_HOME replacement when AUTOLAB_HOME is set", () => {
    vi.stubEnv("AUTOLAB_HOME", "/srv/autolab-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/autolab")).toBe(path.resolve(os.homedir(), "autolab"));
  });
  ...
  it("prefers AUTOLAB_HOME for tilde expansion", () => {
    vi.stubEnv("AUTOLAB_HOME", "/srv/autolab-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/autolab")).toBe(path.resolve("/srv/autolab-home", "autolab"));
