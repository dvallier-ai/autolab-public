...
let ensureAutoLabCliOnPath: typeof import("./path-env.js").ensureAutoLabCliOnPath;

describe("ensureAutoLabCliOnPath", () => {
  const envKeys = [
    "PATH",
    "AUTOLAB_PATH_BOOTSTRAPPED",
    "AUTOLAB_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAutoLabCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling autolab exists", () => {
    const tmp = abs("/tmp/autolab-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "autolab");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.AUTOLAB_PATH_BOOTSTRAPPED;

    ensureAutoLabCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.AUTOLAB_PATH_BOOTSTRAPPED = "1";
    ensureAutoLabCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/autolab-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "autolab");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/autolab-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "autolab");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "autolab");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/autolab-path/case-linuxbrew");
