...
function candidateBinDirs(opts: EnsureAutoLabPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `autolab` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "autolab");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AUTOLAB_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "autolab"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `autolab` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAutoLabCliOnPath(opts: EnsureAutoLabPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AUTOLAB_PATH_BOOTSTRAPPED)) {
    return;
  }
