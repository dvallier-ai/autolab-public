...
vi.mock("./autolab-root.js", () => ({
  resolveAutoLabPackageRoot: vi.fn(async () => null),
  resolveAutoLabPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveAutoLabPackageRoot when available", async () => {
    const autolabRoot = await import("./autolab-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");

    const pkgRoot = abs("fixtures/autolab");
    (
      autolabRoot.resolveAutoLabPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/autolab"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "autolab" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "autolab.mjs"))).resolves.toBe(
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-autolab");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/autolab-bundle");
    (
      autolabRoot.resolveAutoLabPackageRootSync as unknown as ReturnType<typeof vi.fn>
