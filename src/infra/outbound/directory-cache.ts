...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AutoLabConfig | null = null;
  ...
  get(key: string, cfg: AutoLabConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AutoLabConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AutoLabConfig): void {
