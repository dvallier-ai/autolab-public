...
describe("update-startup", () => {
  ...
  let resolveAutoLabPackageRoot: (typeof import("./autolab-root.js"))["resolveAutoLabPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "AUTOLAB_STATE_DIR");
    prevStateDir = process.env.AUTOLAB_STATE_DIR;
    process.env.AUTOLAB_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveAutoLabPackageRoot } = await import("./autolab-root.js"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    vi.mocked(resolveAutoLabPackageRoot).mockResolvedValue("/opt/autolab");
