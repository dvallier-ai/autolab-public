...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__autolab_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "autolab-root");
...
describe("resolveAutoLabPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveAutoLabPackageRootSync } = await import("./autolab-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "autolab");
    const pkgRoot = path.join(project, "node_modules", "autolab");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "autolab" }));

    expect(resolveAutoLabPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "autolab");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "autolab.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "autolab" }));

    expect(resolveAutoLabPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveAutoLabPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-autolab package roots", async () => {
    const { resolveAutoLabPackageRootSync } = await import("./autolab-root.js");

    const pkgRoot = fx("not-autolab");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-autolab" }));

    expect(resolveAutoLabPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveAutoLabPackageRoot } = await import("./autolab-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "autolab" }));

    await expect(resolveAutoLabPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
