...
describe("resolveEffectiveHomeDir", () => {
  it("prefers AUTOLAB_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands AUTOLAB_HOME when set to ~", () => {
    const env = {
      AUTOLAB_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for AUTOLAB_HOME", () => {
    const result = resolveRequiredHomeDir(
      { AUTOLAB_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when AUTOLAB_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ AUTOLAB_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { AUTOLAB_HOME: "/srv/autolab-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/autolab-home")}/x`);
  });
