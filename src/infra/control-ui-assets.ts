...
import { resolveAutoLabPackageRoot, resolveAutoLabPackageRootSync } from "./autolab-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAutoLabPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "autolab" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAutoLabPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
