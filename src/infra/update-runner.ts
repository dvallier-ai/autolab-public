...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "autolab.mjs");
    ...
    const doctorStep = await runStep(
      step("autolab doctor", doctorArgv, gitRoot, { AUTOLAB_UPDATE_IN_PROGRESS: "1" }),
    );
