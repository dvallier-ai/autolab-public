...
function resolveActiveHoursTimezone(cfg: AutoLabConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AutoLabConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: AutoLabConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: AutoLabConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: AutoLabConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: AutoLabConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: AutoLabConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AutoLabConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
