...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.AUTOLAB_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.AUTOLAB_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.AUTOLAB_TEST_TAILSCALE_BINARY;
    } else {
      process.env.AUTOLAB_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
