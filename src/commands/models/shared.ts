...
export async function updateConfig(
  mutator: (cfg: AutoLabConfig) => AutoLabConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AutoLabConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AutoLabConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("autolab agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
