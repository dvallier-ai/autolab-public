...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAutoLabAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("autolab models auth setup-token")}\` or \`${formatCliCommand("autolab configure")}\`.`
          : `Run \`${formatCliCommand("autolab configure")}\` or set an API key env var.`;
