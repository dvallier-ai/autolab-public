...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAutoLabAgentDir: vi.fn().mockReturnValue("/tmp/autolab-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/autolab-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/autolab-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAutoLabAgentDir: mocks.resolveAutoLabAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAutoLabAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/autolab-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/autolab-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/autolab-agent-custom");
