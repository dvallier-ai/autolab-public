...
beforeAll(() => {
  previousProfile = process.env.AUTOLAB_PROFILE;
  process.env.AUTOLAB_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.AUTOLAB_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/autolab-root.js", () => ({
  resolveAutoLabPackageRoot: vi.fn().mockResolvedValue("/tmp/autolab"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/autolab/pnpm-lock.yaml",
      markerPath: "/tmp/autolab/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("AutoLab status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("autolab status --all") ||
          l.includes("autolab --profile isolated status --all") ||
          l.includes("autolab status --all") ||
          l.includes("autolab --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.AUTOLAB_GATEWAY_TOKEN = "abcd1234";
    try {
