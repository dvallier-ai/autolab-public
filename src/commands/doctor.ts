...
function resolveMode(cfg: AutoLabConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AutoLab doctor");

  const root = await resolveAutoLabPackageRoot({
  ...
  let cfg: AutoLabConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("autolab configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("autolab config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("autolab setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("autolab doctor --fix")}" to apply changes.`);
  }
