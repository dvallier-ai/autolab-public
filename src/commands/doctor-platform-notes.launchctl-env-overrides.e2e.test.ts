...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "AUTOLAB_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("AUTOLAB_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv AUTOLAB_GATEWAY_TOKEN");
    expect(message).not.toContain("AUTOLAB_GATEWAY_PASSWORD");
  });
