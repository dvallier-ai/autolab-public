...
import type { AutoLabConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AutoLabConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("autolab security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("autolab configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("autolab config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("autolab doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("autolab config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('autolab config set session.dmScope "per-channel-peer"') +
