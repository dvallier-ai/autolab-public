...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.AUTOLAB_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.AUTOLAB_GATEWAY_PASSWORD;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.autolab.ai/gateway/health",
        "https://docs.autolab.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from autolab.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      "Docs: https://docs.autolab.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "AutoLab update wizard" : "AutoLab configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: AutoLabConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.autolab.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("autolab doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.AUTOLAB_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.AUTOLAB_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.AUTOLAB_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.AUTOLAB_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.AUTOLAB_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.autolab.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
