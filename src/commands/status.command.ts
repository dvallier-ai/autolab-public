...
export async function statusCommand(
  ...
  runtime.log(theme.heading("AutoLab status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("autolab security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("autolab security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.autolab.ai/faq");
  runtime.log("Troubleshooting: https://docs.autolab.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("autolab status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("autolab logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("autolab status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("autolab gateway probe")}`);
  }
}
