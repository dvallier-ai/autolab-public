...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `autolab-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.autolab/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@autolab/voice-call",
        version: "0.0.1",
        autolab: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without autolab.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@autolab/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("autolab.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@autolab/voice-call@0.0.1", "--ignore-scripts"]);
