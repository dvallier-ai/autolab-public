...
import { loadAutoLabPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `autolab-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.AUTOLAB_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "autolab.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.AUTOLAB_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadAutoLabPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AUTOLAB_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAutoLabPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAutoLabPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
