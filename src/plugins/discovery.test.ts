...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `autolab-plugins-${randomUUID()}`);
...
async function withStateDir<T>(stateDir: string, fn: () => Promise<T>) {
  const prev = process.env.AUTOLAB_STATE_DIR;
  ...
  try {
    ...
    if (prev === undefined) {
      ...
      process.env.AUTOLAB_STATE_DIR = prev;
    }
...
describe("discoverAutoLabPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".autolab", "extensions");
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        autolab: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        autolab: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
