...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AutoLabPluginDefinition;
  register?: AutoLabPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AutoLabPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AutoLabPluginDefinition;
...
export function loadAutoLabPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAutoLabPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "autolab/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "autolab/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AutoLabPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as AutoLabPluginModule;
    } catch (err) {
