...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".autolab");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AUTOLAB_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AUTOLAB_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via AUTOLAB_STATE_DIR.
