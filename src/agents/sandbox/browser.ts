...
import {
  ...
  DEFAULT_AUTOLAB_BROWSER_PROFILE_NAME,
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_AUTOLAB_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_AUTOLAB_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_AUTOLAB_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "autolab.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `autolab sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `autolab sandbox recreate --browser --agent ${agentId}`;
          }
          return "autolab sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "autolab.sandboxBrowser": "1" },
    ...
    args.push("-e", `AUTOLAB_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `AUTOLAB_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `AUTOLAB_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `AUTOLAB_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `AUTOLAB_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_AUTOLAB_BROWSER_PROFILE_NAME)
    : null;
