...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses AUTOLAB_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      AUTOLAB_HOME: "/srv/autolab-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/autolab-home"), ".autolab", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".autolab", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("autolab-workspace-");
