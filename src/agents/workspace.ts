...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.AUTOLAB_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".autolab", `workspace-${profile}`);
  }
  return path.join(home, ".autolab", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".autolab";
