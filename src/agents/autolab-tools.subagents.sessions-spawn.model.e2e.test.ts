...
} from "./autolab-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const callGatewayMock = getCallGatewayMock();

type CreateAutoLabTools = (typeof import("./autolab-tools.js"))["createAutoLabTools"];
type CreateAutoLabToolsOpts = Parameters<CreateAutoLabTools>[0];

async function getSessionsSpawnTool(opts: CreateAutoLabToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createAutoLabTools } = await import("./autolab-tools.js");
  const tool = createAutoLabTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("autolab-tools: subagents (sessions_spawn model + thinking)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
