...
import {
  ...
  resolveAutoLabManifestInstall,
  resolveAutoLabManifestOs,
  resolveAutoLabManifestRequires,
...
export function resolveAutoLabMetadata(
  frontmatter: ParsedSkillFrontmatter,
): AutoLabSkillMetadata | undefined {
  const metadataObj = resolveAutoLabManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveAutoLabManifestRequires(metadataObj);
  const install = resolveAutoLabManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveAutoLabManifestOs(metadataObj);
  return {
