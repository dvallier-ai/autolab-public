...
const originalAutoLabStateDir = process.env.AUTOLAB_STATE_DIR;

afterEach(() => {
  if (originalAutoLabStateDir === undefined) {
    delete process.env.AUTOLAB_STATE_DIR;
  } else {
    process.env.AUTOLAB_STATE_DIR = originalAutoLabStateDir;
  }
});
...
describe("installSkill download extraction safety (tar.bz2)", () => {
  ...
  it("rejects tar.bz2 traversal before extraction", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-skills-install-"));
    try {
