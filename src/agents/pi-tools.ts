...
function resolveExecConfig(params: { cfg?: AutoLabConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: AutoLabConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createAutoLabCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createAutoLabReadTool(freshReadTool);
