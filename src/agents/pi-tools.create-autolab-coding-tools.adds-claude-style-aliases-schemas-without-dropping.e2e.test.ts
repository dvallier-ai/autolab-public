...
import { createAutoLabTools } from "./autolab-tools.js";
import { __testing, createAutoLabCodingTools } from "./pi-tools.js";
...
describe("createAutoLabCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createAutoLabTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAutoLabCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "autolab-outside.txt");
