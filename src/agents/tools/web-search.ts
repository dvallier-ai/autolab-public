...
type WebSearchConfig = NonNullable<AutoLabConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AutoLabConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("autolab configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.autolab.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://autolab.ai",
      "X-Title": "AutoLab Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
