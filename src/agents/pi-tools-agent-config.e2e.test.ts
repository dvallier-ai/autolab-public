...
describe("Agent-specific tool filtering", () => {
  ...
  it("defaults apply_patch to workspace-only (blocks traversal)", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-pi-tools-"));
...
*** End Patch`;
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createAutoLabCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createAutoLabCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createAutoLabCodingTools({
      config: cfg,
    ...
    const defaultTools = createAutoLabCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createAutoLabCodingTools({
      config: cfg,
    ...
    const bobTools = createAutoLabCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createAutoLabCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createAutoLabCodingTools({
      config: cfg,
