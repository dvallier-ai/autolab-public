...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-embedded-agent-"));
...
const ensureModels = (cfg: AutoLabConfig) => ensureAutoLabModelsJson(cfg, agentDir) as unknown;
