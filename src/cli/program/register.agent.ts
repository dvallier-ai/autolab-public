...
${formatHelpExamples([
  ['autolab agent --to +15555550123 --message "status update"', "Start a new session."],
  ['autolab agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'autolab agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'autolab agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['autolab agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'autolab agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.autolab.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.autolab.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['autolab agents set-identity --agent main --name "AutoLab" --emoji "🦞"', "Set name + emoji."],
  ["autolab agents set-identity --agent main --avatar avatars/autolab.png", "Set avatar path."],
  [
    "autolab agents set-identity --workspace ~/.autolab/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "autolab agents set-identity --identity-file ~/.autolab/workspace/IDENTITY.md --agent main",
