...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      AUTOLAB_NO_RESPAWN: "1",
      AUTOLAB_STATE_DIR: stateDir,
      AUTOLAB_SKIP_CHANNELS: "1",
      AUTOLAB_SKIP_GMAIL_WATCHER: "1",
      AUTOLAB_SKIP_CRON: "1",
      AUTOLAB_SKIP_BROWSER_CONTROL_SERVER: "1",
      AUTOLAB_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "autolab-entry-bootstrap.mjs");
