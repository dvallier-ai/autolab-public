...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("autolab node install"), formatCliCommand("autolab node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("autolab node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AUTOLAB_LOG_PREFIX: baseEnv.AUTOLAB_LOG_PREFIX ?? "node",
