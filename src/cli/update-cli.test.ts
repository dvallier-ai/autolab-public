...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("autolab-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveAutoLabPackageRoot).mockReset();
    ...
    readPackageName.mockResolvedValue("autolab");
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("autolab-update-wizard");
    const envSnapshot = captureEnv(["AUTOLAB_GIT_DIR"]);
    try {
      setTty(true);
