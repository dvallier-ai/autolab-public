...
import { resolveAutoLabPackageRoot } from "../infra/autolab-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "autolab doctor entry": "Checking doctor entrypoint",
  "autolab doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "autolab";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const AUTOLAB_REPO_URL = "https://github.com/autolab/autolab.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("autolab@")) {
    return trimmed.slice("autolab@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "autolab.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("autolab completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.AUTOLAB_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AUTOLAB_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AUTOLAB_GIT_DIR points at a non-git directory: ${params.dir}. Set AUTOLAB_GIT_DIR to an empty folder or an autolab checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AUTOLAB_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveAutoLabPackageRoot({
  ...
  defaultRuntime.log(theme.heading("AutoLab update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating AutoLab..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this AutoLab install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("autolab doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("autolab gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g autolab@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g autolab@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.AUTOLAB_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.AUTOLAB_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("autolab gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("autolab doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("autolab gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("autolab gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `autolab update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `AUTOLAB_GIT_DIR points at a non-git directory: ${gitDir}. Set AUTOLAB_GIT_DIR to an empty folder or an autolab checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via AUTOLAB_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AutoLab to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["autolab update", "Update a source checkout (git)"],
        ["autolab update --channel beta", "Switch to beta channel (git + npm)"],
        ["autolab update --channel dev", "Switch to dev channel (git + npm)"],
        ["autolab update --tag beta", "One-off update to a dist-tag or version"],
        ["autolab update --no-restart", "Update without restarting the service"],
        ["autolab update --json", "Output result as JSON"],
        ["autolab update --yes", "Non-interactive (accept downgrade prompts)"],
        ["autolab update wizard", "Interactive update wizard"],
        ["autolab --update", "Shorthand for autolab update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run autolab update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.autolab.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["autolab update status", "Show channel + version status."],
          ["autolab update status --json", "JSON output."],
          ["autolab update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.autolab.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
