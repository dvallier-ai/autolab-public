...
describe("skills-cli", () => {
  ...
  describe("integration: loads real skills from bundled directory", () => {
    ...
    beforeAll(async () => {
      envSnapshot = captureEnv(["AUTOLAB_BUNDLED_SKILLS_DIR"]);
      tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-skills-test-"));
      tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "autolab-bundled-skills-test-"));
      process.env.AUTOLAB_BUNDLED_SKILLS_DIR = tempBundledDir;
