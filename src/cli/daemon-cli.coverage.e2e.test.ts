...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    AUTOLAB_STATE_DIR: process.env.AUTOLAB_STATE_DIR,
    AUTOLAB_CONFIG_PATH: process.env.AUTOLAB_CONFIG_PATH,
    AUTOLAB_GATEWAY_PORT: process.env.AUTOLAB_GATEWAY_PORT,
    AUTOLAB_PROFILE: process.env.AUTOLAB_PROFILE,
  };

  beforeEach(() => {
    process.env.AUTOLAB_STATE_DIR = "/tmp/autolab-cli-state";
    process.env.AUTOLAB_CONFIG_PATH = "/tmp/autolab-cli-state/autolab.json";
    delete process.env.AUTOLAB_GATEWAY_PORT;
    delete process.env.AUTOLAB_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.AUTOLAB_STATE_DIR !== undefined) {
      process.env.AUTOLAB_STATE_DIR = originalEnv.AUTOLAB_STATE_DIR;
    } else {
      delete process.env.AUTOLAB_STATE_DIR;
    }

    if (originalEnv.AUTOLAB_CONFIG_PATH !== undefined) {
      process.env.AUTOLAB_CONFIG_PATH = originalEnv.AUTOLAB_CONFIG_PATH;
    } else {
      delete process.env.AUTOLAB_CONFIG_PATH;
    }

    if (originalEnv.AUTOLAB_GATEWAY_PORT !== undefined) {
      process.env.AUTOLAB_GATEWAY_PORT = originalEnv.AUTOLAB_GATEWAY_PORT;
    } else {
      delete process.env.AUTOLAB_GATEWAY_PORT;
    }

    if (originalEnv.AUTOLAB_PROFILE !== undefined) {
      process.env.AUTOLAB_PROFILE = originalEnv.AUTOLAB_PROFILE;
    } else {
      delete process.env.AUTOLAB_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        AUTOLAB_PROFILE: "dev",
        AUTOLAB_STATE_DIR: "/tmp/autolab-daemon-state",
        AUTOLAB_CONFIG_PATH: "/tmp/autolab-daemon-state/autolab.json",
        AUTOLAB_GATEWAY_PORT: "19001",
      },
