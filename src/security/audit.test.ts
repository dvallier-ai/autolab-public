...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: AutoLabConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.AUTOLAB_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevPassword === undefined) {
        ...
        process.env.AUTOLAB_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: "/Users/test/Dropbox/.danv-intel/autolab.json",
    });
