...
function summarizeGroupPolicy(cfg: AutoLabConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: AutoLabConfig): boolean {
...
function collectModels(cfg: AutoLabConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: AutoLabConfig): Set<string> {
  const baseCfg: AutoLabConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: AutoLabConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AutoLabConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AutoLabConfig): boolean {
...
function isBrowserEnabled(cfg: AutoLabConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AutoLabConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep AUTOLAB_STATE_DIR on a local-only volume and re-run "${formatCliCommand("autolab security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer AUTOLAB_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const autolabGatewayToken =
    typeof env.AUTOLAB_GATEWAY_TOKEN === "string" && env.AUTOLAB_GATEWAY_TOKEN.trim()
      ? env.AUTOLAB_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-autolab-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AutoLabConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
