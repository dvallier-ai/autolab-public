...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "autolab-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".autolab", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AutoLabConfig> = {},
): AutoLabConfig {
  const base: AutoLabConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".autolab", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
