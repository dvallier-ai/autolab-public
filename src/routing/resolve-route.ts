...
function listAgents(cfg: AutoLabConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: AutoLabConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AutoLabConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AutoLabConfig, EvaluatedBindingsCache>();
