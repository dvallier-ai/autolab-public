...
describe("canvas host state dir defaults", () => {
  ...
  it("uses AUTOLAB_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("AutoLab Canvas");
    } finally {
      await handler.close();
