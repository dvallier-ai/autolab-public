...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.AUTOLAB_CONFIG_PATH;
  const prevDisableCache = process.env.AUTOLAB_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "autolab-plugin-http-auth-test-"));
  const configPath = path.join(dir, "autolab.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.AUTOLAB_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.AUTOLAB_DISABLE_CONFIG_CACHE;
    } else {
      process.env.AUTOLAB_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
