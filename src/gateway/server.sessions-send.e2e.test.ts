...
const { createAutoLabTools } = await import("../agents/autolab-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["AUTOLAB_GATEWAY_PORT", "AUTOLAB_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.AUTOLAB_GATEWAY_PORT = String(gatewayPort);
  process.env.AUTOLAB_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("AUTOLAB_CONFIG_PATH missing in gateway test environment");
    }
