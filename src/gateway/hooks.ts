...
export function resolveHooksConfig(cfg: AutoLabConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AutoLabConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-autolab-token"] === "string"
      ? req.headers["x-autolab-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
