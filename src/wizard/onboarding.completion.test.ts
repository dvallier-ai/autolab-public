...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "autolab",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/autolab.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("autolab");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "autolab");
