...
beforeEach(() => {
  ...
  vi.stubEnv("AUTOLAB_TEST_FAST", "1");
});
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { AUTOLAB_STATE_DIR: process.env.AUTOLAB_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.AUTOLAB_STATE_DIR === undefined) {
      delete process.env.AUTOLAB_STATE_DIR;
    } else {
      process.env.AUTOLAB_STATE_DIR = snapshot.AUTOLAB_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "autolab-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "autolab-memory-flush-"));
  });
