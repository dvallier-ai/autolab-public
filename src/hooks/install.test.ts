...
const fixtureRoot = path.join(os.tmpdir(), `autolab-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@autolab/test-hooks",
        version: "0.0.1",
        autolab: { hooks: ["./hooks/one-hook"] },
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@autolab/test-hooks@0.0.1", "--ignore-scripts"]);
