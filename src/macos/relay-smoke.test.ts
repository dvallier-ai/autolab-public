...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { AUTOLAB_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { AUTOLAB_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports AUTOLAB_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { AUTOLAB_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { AUTOLAB_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { AUTOLAB_SMOKE: "nope" })).toBe("qr");
  });
});
