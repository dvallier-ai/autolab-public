...
declare const __AUTOLAB_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __AUTOLAB_VERSION__ === "string" && __AUTOLAB_VERSION__) ||
  process.env.AUTOLAB_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `autolab --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.AUTOLAB_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.AUTOLAB_GATEWAY_BIND ??
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (AUTOLAB_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
