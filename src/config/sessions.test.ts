...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "autolab-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-autolab",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-autolab#general");
  });
  ...
  it("derives session transcripts dir from AUTOLAB_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { AUTOLAB_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.AUTOLAB_STATE_DIR = "/custom/state";
    try {
