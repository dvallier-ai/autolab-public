...
function applyConfigEnv(cfg: AutoLabConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AutoLabConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as AutoLabConfig, deps.env);
      }
  ...
  async function writeConfigFile(cfg: AutoLabConfig) {
    clearConfigCache();
...
export async function writeConfigFile(cfg: AutoLabConfig): Promise<void> {
  clearConfigCache();
