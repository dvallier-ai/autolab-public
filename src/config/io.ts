...
import type { AutoLabConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareAutoLabVersions } from "./version.js";
...
function coerceConfig(value: unknown): AutoLabConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AutoLabConfig;
}
...
function stampConfigVersion(cfg: AutoLabConfig): AutoLabConfig {
...
function warnIfConfigFromFuture(cfg: AutoLabConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAutoLabVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AutoLab (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AutoLabConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AutoLabConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AutoLabConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AutoLabConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AutoLabConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.AUTOLAB_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.AUTOLAB_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.AUTOLAB_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.AUTOLAB_WATCH_SESSION === "string" &&
        deps.env.AUTOLAB_WATCH_SESSION.trim().length > 0
          ? deps.env.AUTOLAB_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.AUTOLAB_WATCH_COMMAND === "string" &&
        deps.env.AUTOLAB_WATCH_COMMAND.trim().length > 0
          ? deps.env.AUTOLAB_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `AUTOLAB_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AUTOLAB_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AUTOLAB_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): AutoLabConfig {
