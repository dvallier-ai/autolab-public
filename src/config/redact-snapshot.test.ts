...
import type { ConfigFileSnapshot } from "./types.autolab.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.autolab/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/autolab/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/autolab/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/autolab/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/autolab/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    ...
    const hints = mapSensitivePaths(AutoLabSchema, "", {});
