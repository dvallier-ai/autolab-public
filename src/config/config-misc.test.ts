...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = AutoLabSchema.safeParse({
      $schema: "https://autolab.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://autolab.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = AutoLabSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhook and cron.webhookToken", () => {
    const res = AutoLabSchema.safeParse({
      cron: {
        enabled: true,
